...
from .fixture_order_flow import DEFAULT_ORDER_FLOW_FIXTURE
...
def _load_bars_by_time(fixture_path: Path) -> dict[str, dict[str, Any]]:
...
class FixtureOrderBookProvider:
    ...
    def build_envelopes(self, *, as_of_time_ns: int | None = None) -> list[dict[str, Any]]:
        ...
        bars_by_time = _load_bars_by_time(DEFAULT_ORDER_FLOW_FIXTURE)
        ...
        for index, snapshot in enumerate(snapshots):
            ...
            if prev_snapshot is None:
                ...
                prev_bar = bars_by_time.get(str(prev_snapshot.get("event_time", "")))
                ...
                buying_volume = (
                    float(curr_bar["buying_volume"]) if curr_bar and curr_bar.get("buying_volume") is not None else None
                )
                selling_volume = (
                    float(curr_bar["selling_volume"]) if curr_bar and curr_bar.get("selling_volume") is not None else None
                )
