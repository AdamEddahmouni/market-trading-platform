"""Nasdaq Trader public Reg SHO threshold file transport. No FINRA credential."""
...
THRESHOLD_URL_TEMPLATE = "https://www.nasdaqtrader.com/dynamic/SymDir/regsho/nasdaqth{yyyymmdd}.txt"
...
class NasdaqTransport:
    ...
    def fetch_threshold_file(self, trade_date: str, *, timeout: float = DEFAULT_TIMEOUT_S) -> bytes:
        ...
        url = THRESHOLD_URL_TEMPLATE.format(yyyymmdd=compact)
        last_error: BaseException | None = None
        for attempt in range(MAX_RETRIES):
            self._throttle()
            try:
                body = self._requester(url, {"User-Agent": self.user_agent, "Accept": "text/plain"}, timeout)
                ...
                if exc.code == 404:
                    raise OSError("NASDAQ_THRESHOLD_FILE_MISSING") from exc
                if exc.code >= 500 and attempt + 1 < MAX_RETRIES:
                    self._sleeper(0.2 * (2 ** attempt))
                    continue
                raise OSError(f"SOURCE_UNAVAILABLE") from exc
    ...
    def _throttle(self) -> None:
        with self._global_lock:
            elapsed = time.monotonic() - NasdaqTransport._last_request
            wait = self.min_interval_s - elapsed
            if wait > 0:
                self._sleeper(wait)
            NasdaqTransport._last_request = time.monotonic()
