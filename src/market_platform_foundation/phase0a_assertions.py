"""Phase 0A data-feasibility assertion registry and evaluator."""
...
MANDATORY_IDS = ("DF-001", "DF-002")

_ROLES = {
    "DF-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "DF-002": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
}
...
def build_registry(path: Path) -> dict[str, object]:
    ...
    if not isinstance(raw, dict) or raw.get("registry_version") != "1.0.0":
        raise ValueError("unsupported phase0a assertion registry")
