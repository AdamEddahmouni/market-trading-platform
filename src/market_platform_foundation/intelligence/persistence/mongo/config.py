"""Mongo repository configuration (BUILD 04.5)."""
...
DEFAULT_MONGODB_DATABASE = "imp_intelligence"
DEFAULT_SERVER_SELECTION_TIMEOUT_MS = 2000
TEST_DATABASE_PREFIX = "imp_test_"
...
def redact_mongo_uri(uri: str) -> str:
    """Redact credential-bearing Mongo connection URIs."""
    if not uri:
        return uri
...
class MongoRepositoryConfig:
    """Typed Mongo persistence configuration."""
    ...
    @classmethod
    def from_env(cls, environ: Mapping[str, str] | None = None) -> MongoRepositoryConfig:
        env = environ or os.environ
        uri = str(env.get("IMP_MONGODB_URI", "")).strip()
        if not uri:
            raise ValueError("IMP_MONGODB_URI_REQUIRED")
        database = str(env.get("IMP_MONGODB_DATABASE", DEFAULT_MONGODB_DATABASE)).strip()
        timeout_raw = str(env.get("IMP_MONGODB_SERVER_SELECTION_TIMEOUT_MS", "")).strip()
        timeout_ms = DEFAULT_SERVER_SELECTION_TIMEOUT_MS
        if timeout_raw:
            timeout_ms = int(timeout_raw)
        app_name = str(env.get("IMP_MONGODB_APPLICATION_NAME", "imp-intelligence")).strip() or None
        return cls(
            uri=uri,
    ...
    @classmethod
    def from_test_env(cls, environ: Mapping[str, str] | None = None) -> MongoRepositoryConfig:
        env = environ or os.environ
    ...
    def __repr__(self) -> str:
        return (
            f"MongoRepositoryConfig(uri={redact_mongo_uri(self.uri)!r}, "
