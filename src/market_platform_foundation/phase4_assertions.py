"""Phase 4 runtime quality and state assertion registry and evaluator."""
...
MANDATORY_IDS = ("TC-001", "TC-003", "DET-001", "SAFE-003")

_ROLES = {
    "TC-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "TC-003": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
...
def build_registry(path: Path) -> dict[str, object]:
    ...
    if not isinstance(raw, dict) or raw.get("registry_version") != "1.0.0":
        raise ValueError("unsupported phase4 assertion registry")
