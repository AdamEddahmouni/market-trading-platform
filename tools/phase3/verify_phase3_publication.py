"""Verify published Phase 3 PASS bindings."""
...
FINAL_RESULT_PATH = ROOT / "evidence/phase3/postreview-pass/phase3.final_acceptance_result.json"
INDEX_PATH = ROOT / "evidence/phase3/postreview-pass/phase3.acceptance_index.json"


def main() -> int:
    all_ok = True
    print("=== Phase 3 publication verification ===")
    ...
    if not isinstance(manifest, dict):
        ...
        if phase3_status != "PASS":
            all_ok = False
            print(f" FAIL authority.phase3_status: {phase3_status}")
        else:
            print(" PASS authority.phase3_status: PASS")
    ...
    if not PUBLICATION_PATH.is_file():
        ...
        if not isinstance(publication, dict) or publication.get("status") != "PUBLISHED":
            ...
            if isinstance(bound, dict):
                ...
                current_hash = sha256_bytes(AUTHORITY_PATH.read_bytes())
                phase4_status = manifest.get("phase4_status")
                if phase4_status:
                    print(f" NOTE manifest extended with phase4_status={phase4_status}")
                    print(f" NOTE current_manifest_hash={current_hash}")
                    print(f" NOTE publication_era_hash={recorded}")
                    if not recorded:
                        all_ok = False
                        print(" FAIL authority_manifest_at_publication hash missing")
                    else:
