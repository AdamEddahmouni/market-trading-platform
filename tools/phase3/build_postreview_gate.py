"""Build Phase 3 postreview gate artifacts."""
...
from tools.phase3.acceptance_algorithms import (
...
DEFAULT_BUNDLE = ROOT / "evidence/phase3" / RUN_ID
DEFAULT_POSTREVIEW = ROOT / "evidence/phase3/postreview-pass"
DEFAULT_ADVERSARIAL = ROOT / "evidence/phase3/review-runs/ADVERSARIAL-PASS-PHASE3-ADAPTER"
DEFAULT_INTEGRITY = ROOT / "evidence/phase3/review-runs/INTEGRITY-PASS-PHASE3-ADAPTER"
...
GOVERNANCE_PATHS = {
    ...
    "phase3.design_specification": (
        "docs/superpowers/specs/2026-08-15-phase-3-verified-historical-adapter-design.md"
    ),
    "phase3.governance_approvals": (
        "docs/superpowers/governance/2026-08-15-phase-3-governance-approvals.json"
    ),
    "phase3.governance_plan": (
        "docs/superpowers/plans/2026-08-15-phase-3-verified-historical-adapter.md"
    ),
    "phase3.implementation_activation": (
        "docs/superpowers/governance/2026-08-15-phase-3-implementation-activation.json"
    ),
    "phase3.implementation_authorization": (
        "docs/superpowers/governance/2026-08-15-phase-3-implementation-authorization.json"
    ),
}
...
def load_review_output(review_dir: Path) -> dict[str, object]:
    return load_json(review_dir / "phase3.ai_review_output.json")
...
def build_approval_records(*, approved_at: str) -> dict[str, object]:
    return {
                    ...
                    canonical_bytes(
                        {
                            ...
                            "approved_logical_id": "phase3.candidate_evidence_root",
                            "approved_sha256": CANDIDATE_ROOT,
                            "approval_scope": "PHASE_3_CANDIDATE_EVIDENCE_ROOT_ACCEPTANCE",
                            "status": "APPROVED",
                        }
                    )
                ),
        ...
        "logical_id": "phase3.approval_records",
...
def build_coverage(adversarial: dict[str, object], integrity: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "logical_id": "phase3.ai_review_coverage",
...
def build_ai_review_runs(
    ...
    adv_run = load_json(adversarial_dir / "phase3.ai_review_run.json")
    int_run = load_json(integrity_dir / "phase3.ai_review_run.json")
    return {
        "candidate_evidence_root": CANDIDATE_ROOT,
        "review_runs": sorted(
            [
                {
                    ...
                    "output_repository_relative_path": repo_relative(
                        adversarial_dir / "phase3.ai_review_output.json"
                    ),
                    ...
                    "run_record_repository_relative_path": repo_relative(
                        adversarial_dir / "phase3.ai_review_run.json"
                    ),
...
def build_index_members(bundle: Path, postreview: Path) -> list[dict[str, object]]:
    ...
    postreview_map = {
        "phase3.ai_review_runs": postreview / "phase3.ai_review_runs.json",
        "phase3.ai_review_coverage": postreview / "phase3.ai_review_coverage.json",
        "phase3.approval_records": postreview / "phase3.approval_records.json",
    }
    ...
    candidate_ids = [row["logical_id"] for row in members if row["logical_id"] != "phase3.candidate_evidence_root"]
...
def build_final_result(
    *,
    ...
    provisional = {
        ...
        "logical_id": "phase3.final_acceptance_result",
        "outcome": outcome,
...
def build_all(
    *,
    ...
    write_canonical(postreview / "phase3.approval_records.json", build_approval_records(approved_at=approved_at))
    coverage_doc = build_coverage(adversarial, integrity)
    write_canonical(postreview / "phase3.ai_review_coverage.json", coverage_doc)
    write_canonical(
        postreview / "phase3.ai_review_runs.json",
    ...
    write_canonical(postreview / "phase3.acceptance_index.json", index_doc)
    ...
    write_canonical(postreview / "phase3.final_acceptance_result.json", final_doc)
    return {
