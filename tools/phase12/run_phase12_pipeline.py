"""Build Phase 12 large_transactions whale family acceptance evidence."""
...
REGISTRY_PATH = ROOT / "manifests/phase12/assertion-predicates.json"
FIXTURE_MANIFEST = ROOT / "tests/fixtures/providers/large_transactions/admission_manifest.json"
...
from market_platform_foundation.phase12_assertions import aggregate_status, build_registry, create_run_manifest, evaluate_run
...
from market_platform_foundation.providers.whale_ledger import WHALE_ENTITLED_LARGE_TRANSACTIONS, build_combined_fixture_ledger
...
def _safe003_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_12_SAFE003_REPORT",
        "logical_id": "phase12.safe003_report",
...
def _fixture_report() -> dict[str, object]:
    ...
    if not isinstance(claims, list):
        ...
        for claim in ("size_normalization", "rolling_volume_reference", "direction_ambiguity"):
            if claim not in claims:
                failures.append(f"MISSING_{claim}")
    return {
        "artifact_type": "PHASE_12_FIXTURE_REPORT",
        "failures": failures,
        "logical_id": "phase12.fixture_report",
        "status": "PASS" if not failures else "FAIL",
    }


def _large_transactions_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_12_LARGE_TRANSACTIONS_REPORT",
        "deterministic_envelope_ids": deterministic,
        "event_count": len(first_ids),
        "logical_id": "phase12.large_transactions_report",
...
def _pit_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_12_PIT_REPORT",
        "event_count": len(result.events),
        "logical_id": "phase12.pit_report",
...
def _whale_report() -> dict[str, object]:
    ...
    failures: list[str] = []
    if nvda.get("status") != "available" or nvda.get("reason_code") != WHALE_ENTITLED_LARGE_TRANSACTIONS:
    ...
    return {
        "artifact_type": "PHASE_12_WHALE_REPORT",
        "failures": failures,
        "logical_id": "phase12.whale_report",
        "status": "PASS" if not failures else "FAIL",
    }
...
def _ui_report() -> dict[str, object]:
    ...
    failures: list[str] = []
    if by_id.get("whale.large_transactions", {}).get("state") != "AVAILABLE":
    ...
    return {
        "artifact_type": "PHASE_12_UI_REPORT",
        "failures": failures,
        "logical_id": "phase12.ui_report",
        "status": "PASS" if not failures else "FAIL",
    }
...
def build_evidence(output_dir: Path) -> dict[str, object]:
    ...
    large_transactions = _large_transactions_report()
    pit = _pit_report()
    whale = _whale_report()
    ui = _ui_report()
    ...
    write_canonical_json(output_dir / "large-transactions-report.json", large_transactions)
    ...
    observations = {
        "P12-FIX-001": {"failures": fixture.get("failures"), "reason_codes": fixture.get("failures", []), "status": fixture.get("status")},
        "P12-LT-001": {"reason_codes": [] if large_transactions.get("status") == "PASS" else ["P12-LT-001-FAIL"], "status": large_transactions.get("status")},
        "P12-PIT-001": {"reason_codes": [] if pit.get("status") == "PASS" else ["P12-PIT-001-FAIL"], "status": pit.get("status")},
        "P12-WHALE-001": {"failures": whale.get("failures"), "reason_codes": whale.get("failures", []), "status": whale.get("status")},
        "P12-UI-001": {"failures": ui.get("failures"), "reason_codes": ui.get("failures", []), "status": ui.get("status")},
    ...
    manifest_inputs = {
        ...
        "subject_manifest_hash": sha256_bytes(canonical_bytes({"track": "phase12", "root_id": "ROOT-2E7C91F4"})),
        "tool_versions": ["phase12.run_phase12_pipeline/1.0.0"],
    }
