...
def publish(*, publication_path: Path, run_id: str, candidate_root: str) -> dict[str, object]:
    final_path = POSTREVIEW_DIR / "phase12.final_acceptance_result.json"
    index_path = POSTREVIEW_DIR / "phase12.acceptance_index.json"
    ...
    if manifest.get("phase12_status") == "PASS":
        raise ValueError("Phase 12 PASS already published")

    authority_at_publication = write_canonical_json(
        AUTHORITY_PATH,
        {**manifest, "phase12_status": "PASS"},
    )
    ...
    publication_doc = {
        "acceptance_index": {
            ...
            "repository_relative_path": "evidence/phase12/postreview-pass/phase12.acceptance_index.json",
            "root_hash": index_doc.get("root_hash"),
            "sha256": sha256_bytes(index_path.read_bytes()),
        },
        "artifact_type": "PHASE_12_PASS_PUBLICATION",
        ...
        "effect": (
            "Publishes Phase 12 PASS for fixture-first large_transactions whale family on "
            "ADMITTED-LARGE-PRINTS-NVDA-001."
        ),
        "final_acceptance_result": {
            "final_result_id": final_doc.get("final_result_id"),
            "outcome": final_doc.get("outcome"),
            "repository_relative_path": "evidence/phase12/postreview-pass/phase12.final_acceptance_result.json",
            "sha256": sha256_bytes(final_path.read_bytes()),
        },
        "logical_id": "phase12.pass_publication",
        "principal_id": "PROJECT-PRINCIPAL-001",
        "published_at": "2026-08-18T12:35:00.000000000Z",
...
def main() -> int:
    ...
    if args.run_id == RUN_ID or args.candidate_root == CANDIDATE_ROOT:
        bundle = ROOT / f"evidence/phase12/{args.run_id}" if args.run_id != RUN_ID else None
