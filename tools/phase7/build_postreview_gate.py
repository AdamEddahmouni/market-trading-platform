"""Build Phase 7 postreview gate artifacts."""
...
from tools.phase7.acceptance_algorithms import (
...
DEFAULT_POSTREVIEW = ROOT / "evidence/phase7/postreview-pass"
DEFAULT_ADVERSARIAL = ROOT / "evidence/phase7/review-runs/ADVERSARIAL-PASS-PHASE7-RISK-SIMULATION"
DEFAULT_INTEGRITY = ROOT / "evidence/phase7/review-runs/INTEGRITY-PASS-PHASE7-RISK-SIMULATION"
...
GOVERNANCE_PATHS = {
    ...
    "phase6.pass_publication": "docs/superpowers/governance/2026-08-15-phase-6-pass-publication.json",
    "phase7.design_specification": (
        "docs/superpowers/specs/2026-08-16-phase-7-risk-simulation-accounting-design.md"
    ),
    "phase7.governance_approvals": (
        "docs/superpowers/governance/2026-08-16-phase-7-governance-approvals.json"
    ),
    "phase7.governance_plan": (
        "docs/superpowers/plans/2026-08-16-phase-7-risk-simulation-accounting.md"
    ),
    "phase7.implementation_activation": (
        "docs/superpowers/governance/2026-08-16-phase-7-implementation-activation.json"
    ),
    "phase7.implementation_authorization": (
        "docs/superpowers/governance/2026-08-16-phase-7-implementation-authorization.json"
    ),
}
...
def load_review_output(review_dir: Path) -> dict[str, object]:
    return load_json(review_dir / "phase7.ai_review_output.json")
...
def build_approval_records(*, approved_at: str, candidate_root: str) -> dict[str, object]:
    return {
                    ...
                    canonical_bytes(
                        {
                            ...
                            "approved_logical_id": "phase7.candidate_evidence_root",
                            "approved_sha256": candidate_root,
                            "approval_scope": "PHASE_7_CANDIDATE_EVIDENCE_ROOT_ACCEPTANCE",
                            "status": "APPROVED",
                        }
                    )
                ),
        ...
        "logical_id": "phase7.approval_records",
...
def build_coverage(
    ...
    *,
    ...
    return {
        ...
        "logical_id": "phase7.ai_review_coverage",
...
def build_ai_review_runs(
    ...
    *,
    ...
    adv_run = load_json(adversarial_dir / "phase7.ai_review_run.json")
    int_run = load_json(integrity_dir / "phase7.ai_review_run.json")
    return {
        "candidate_evidence_root": candidate_root,
        "review_runs": sorted(
            [
                {
                    "completed_at": adv_run.get("completed_at", "2026-08-16T01:30:00.000000000Z"),
                    "output_repository_relative_path": repo_relative(
                        adversarial_dir / "phase7.ai_review_output.json"
                    ),
                    ...
                    "run_record_repository_relative_path": repo_relative(
                        adversarial_dir / "phase7.ai_review_run.json"
                    ),
                    "terminal_state": adv_run["terminal_state"],
                },
                {
                    "completed_at": int_run.get("completed_at", "2026-08-16T01:30:00.000000000Z"),
                    "output_repository_relative_path": repo_relative(
                        integrity_dir / "phase7.ai_review_output.json"
                    ),
                    "review_class": integrity["review_class"],
                    "review_output_hash": int_run["review_output_hash"],
                    "review_run_id": int_run["review_run_id"],
                    "run_record_repository_relative_path": repo_relative(
                        integrity_dir / "phase7.ai_review_run.json"
                    ),
...
def build_index_members(bundle: Path, postreview: Path) -> list[dict[str, object]]:
    ...
    postreview_map = {
        "phase7.ai_review_runs": postreview / "phase7.ai_review_runs.json",
        "phase7.ai_review_coverage": postreview / "phase7.ai_review_coverage.json",
        "phase7.approval_records": postreview / "phase7.approval_records.json",
    }
    ...
    candidate_ids = [
        row["logical_id"] for row in members if row["logical_id"] != "phase7.candidate_evidence_root"
    ]
...
def build_final_result(
    *,
    ...
    provisional = {
        ...
        "logical_id": "phase7.final_acceptance_result",
        "outcome": outcome,
...
def build_all(
    *,
    ...
    write_canonical(
        postreview / "phase7.approval_records.json",
    ...
    write_canonical(postreview / "phase7.ai_review_coverage.json", coverage_doc)
    write_canonical(
        postreview / "phase7.ai_review_runs.json",
    ...
    write_canonical(postreview / "phase7.acceptance_index.json", index_doc)
    ...
    write_canonical(postreview / "phase7.final_acceptance_result.json", final_doc)
    return {
...
def main() -> int:
    ...
    parser.add_argument("--approved-at", default="2026-08-16T01:30:00.000000000Z")
    parser.add_argument("--completed-at", default="2026-08-16T01:32:00.000000000Z")
