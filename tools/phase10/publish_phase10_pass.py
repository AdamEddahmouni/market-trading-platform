...
def publish(*, publication_path: Path, run_id: str, candidate_root: str) -> dict[str, object]:
    final_path = POSTREVIEW_DIR / "phase10.final_acceptance_result.json"
    index_path = POSTREVIEW_DIR / "phase10.acceptance_index.json"
    ...
    if manifest.get("phase10_status") == "PASS":
        raise ValueError("Phase 10 PASS already published")

    authority_at_publication = write_canonical_json(
        AUTHORITY_PATH,
        {**manifest, "phase10_status": "PASS"},
    )
    ...
    publication_doc = {
        "acceptance_index": {
            ...
            "repository_relative_path": "evidence/phase10/postreview-pass/phase10.acceptance_index.json",
            "root_hash": index_doc.get("root_hash"),
            "sha256": sha256_bytes(index_path.read_bytes()),
        },
        "artifact_type": "PHASE_10_PASS_PUBLICATION",
        ...
        "effect": (
            "Publishes Phase 10 PASS for fixture-first order_flow whale family on "
            "ADMITTED-CVD-NVDA-ORDERFLOW-001."
        ),
        "final_acceptance_result": {
            "final_result_id": final_doc.get("final_result_id"),
            "outcome": final_doc.get("outcome"),
            "repository_relative_path": "evidence/phase10/postreview-pass/phase10.final_acceptance_result.json",
            "sha256": sha256_bytes(final_path.read_bytes()),
        },
        "logical_id": "phase10.pass_publication",
        "principal_id": "PROJECT-PRINCIPAL-001",
        "published_at": "2026-08-17T12:35:00.000000000Z",
...
def main() -> int:
    ...
    if args.run_id == RUN_ID or args.candidate_root == CANDIDATE_ROOT:
        bundle = ROOT / f"evidence/phase10/{args.run_id}" if args.run_id != RUN_ID else None
