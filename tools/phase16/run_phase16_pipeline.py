"""Build Phase 16 fund_etf_cross_asset whale family acceptance evidence."""
...
REGISTRY_PATH = ROOT / "manifests/phase16/assertion-predicates.json"
FIXTURE_MANIFEST = ROOT / "tests/fixtures/providers/fund_etf/admission_manifest.json"
EVALUATED_AT = "2026-08-17T23:45:00.000000000Z"
...
from market_platform_foundation.phase16_assertions import aggregate_status, build_registry, create_run_manifest, evaluate_run
...
def _safe003_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_16_SAFE003_REPORT",
        "logical_id": "phase16.safe003_report",
...
def _fixture_report() -> dict[str, object]:
    ...
    if not isinstance(claims, list):
        ...
        for claim in ("etf_flow_proxy", "cross_asset_correlation", "regime_label", "flow_direction"):
            if claim not in claims:
                failures.append(f"MISSING_{claim}")
    return {
        "artifact_type": "PHASE_16_FIXTURE_REPORT",
        "failures": failures,
        "logical_id": "phase16.fixture_report",
        "status": "PASS" if not failures else "FAIL",
    }


def _fund_etf_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_16_FUND_ETF_REPORT",
        "deterministic_envelope_ids": deterministic,
        "event_count": len(first_ids),
        "logical_id": "phase16.fund_etf_report",
        "status": "PASS" if deterministic else "FAIL",
    }


def _pit_report() -> dict[str, object]:
    cutoff = iso_to_epoch_ns("2026-07-21T18:30:00.000000000Z")
    provider = FixtureFundEtfProvider(fixture_path=DEFAULT_FUND_ETF_FIXTURE)
    result = provider.fetch_fund_etf_activity("NVDA", as_of_time_ns=cutoff)
    return {
        "artifact_type": "PHASE_16_PIT_REPORT",
        "event_count": len(result.events),
        "logical_id": "phase16.pit_report",
...
def _whale_report() -> dict[str, object]:
    ...
    boxl = query_institutional_evidence(
        FUND_ETF_FAMILY,
        prediction_cutoff=cutoff,
        instrument_id="BOXL",
    )
    failures: list[str] = []
    if nvda.get("status") != "available" or nvda.get("reason_code") != WHALE_ENTITLED_FUND_ETF:
        failures.append("NVDA_UNAVAILABLE")
    if boxl.get("status") != "unavailable" or boxl.get("reason_code") != NO_ENTITLED_SOURCE:
        failures.append("BOXL_OVERCLAIM")
    configure_institutional_ledger(None)
    return {
        "artifact_type": "PHASE_16_WHALE_REPORT",
        "failures": failures,
        "logical_id": "phase16.whale_report",
        "status": "PASS" if not failures else "FAIL",
    }
...
def _ui_report() -> dict[str, object]:
    ...
    failures: list[str] = []
    if by_id.get("whale.fund_etf_cross_asset", {}).get("state") != "AVAILABLE":
    ...
    return {
        "artifact_type": "PHASE_16_UI_REPORT",
        "failures": failures,
        "logical_id": "phase16.ui_report",
        "status": "PASS" if not failures else "FAIL",
    }
...
def build_evidence(output_dir: Path) -> dict[str, object]:
    ...
    write_canonical_json(output_dir / "fund-etf-report.json", fund_etf)
    ...
    observations = {
        "P16-FIX-001": {"failures": fixture.get("failures"), "reason_codes": fixture.get("failures", []), "status": fixture.get("status")},
        "P16-FUND-001": {"reason_codes": [] if fund_etf.get("status") == "PASS" else ["P16-FUND-001-FAIL"], "status": fund_etf.get("status")},
        "P16-PIT-001": {"reason_codes": [] if pit.get("status") == "PASS" else ["P16-PIT-001-FAIL"], "status": pit.get("status")},
        "P16-WHALE-001": {"failures": whale.get("failures"), "reason_codes": whale.get("failures", []), "status": whale.get("status")},
        "P16-UI-001": {"failures": ui.get("failures"), "reason_codes": ui.get("failures", []), "status": ui.get("status")},
        "SAFE-003": {"reason_codes": safe003.get("reason_codes"), "status": safe003.get("status")},
    }

    members = {
        "phase16.fixture_report": "fixture-report.json",
        "phase16.fund_etf_report": "fund-etf-report.json",
        "phase16.pit_report": "pit-report.json",
        "phase16.safe003_report": "safe003-report.json",
        "phase16.ui_report": "ui-report.json",
        "phase16.whale_report": "whale-report.json",
    }
    selected_evidence = []
    ...
    manifest_inputs = {
        ...
        "subject_manifest_hash": sha256_bytes(canonical_bytes({"track": "phase16", "root_id": "ROOT-FUND-ETF-LANE"})),
        "tool_versions": ["phase16.run_phase16_pipeline/1.0.0"],
    }
    ...
    bundle_members = {
        **members,
        "phase16.assertion_aggregate": "assertion-aggregate.json",
        "phase16.assertion_registry": "assertion_registry.json",
        "phase16.assertion_results": "assertion-results.json",
        "phase16.assertion_run_manifest": "assertion-run-manifest.json",
    }
    candidate_members = []
    ...
    candidate_body = {
        ...
        "logical_id": "phase16.candidate_evidence_root",
        "run_id": run_id,
    }
