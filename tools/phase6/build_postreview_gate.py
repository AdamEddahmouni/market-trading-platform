"""Build Phase 6 postreview gate artifacts."""
...
from tools.phase6.acceptance_algorithms import (
...
DEFAULT_POSTREVIEW = ROOT / "evidence/phase6/postreview-pass"
DEFAULT_ADVERSARIAL = ROOT / "evidence/phase6/review-runs/ADVERSARIAL-PASS-PHASE6-STRATEGY"
DEFAULT_INTEGRITY = ROOT / "evidence/phase6/review-runs/INTEGRITY-PASS-PHASE6-STRATEGY"
...
GOVERNANCE_PATHS = {
    ...
    "phase5r.pass_publication": "docs/superpowers/governance/2026-08-15-phase-5r-pass-publication.json",
    "phase6.design_specification": (
        "docs/superpowers/specs/2026-08-15-phase-6-preregistered-strategy-design.md"
    ),
    "phase6.governance_approvals": (
        "docs/superpowers/governance/2026-08-15-phase-6-governance-approvals.json"
    ),
    "phase6.governance_plan": (
        "docs/superpowers/plans/2026-08-15-phase-6-preregistered-strategy.md"
    ),
    "phase6.implementation_activation": (
        "docs/superpowers/governance/2026-08-15-phase-6-implementation-activation.json"
    ),
    "phase6.implementation_authorization": (
        "docs/superpowers/governance/2026-08-15-phase-6-implementation-authorization.json"
    ),
}
...
def resolve_phase6_bundle() -> tuple[str, str, Path]:
    evidence_dir = ROOT / "evidence/phase6"
    if not evidence_dir.is_dir():
        raise ValueError("evidence/phase6 missing")
    bundles = [
        path
    ...
    if len(bundles) != 1:
        raise ValueError(f"expected exactly one phase6 bundle, found {len(bundles)}")
    bundle = bundles[0]
...
def load_review_output(review_dir: Path) -> dict[str, object]:
    return load_json(review_dir / "phase6.ai_review_output.json")
...
def build_approval_records(*, approved_at: str, candidate_root: str) -> dict[str, object]:
    return {
                    ...
                    canonical_bytes(
                        {
                            ...
                            "approved_logical_id": "phase6.candidate_evidence_root",
                            "approved_sha256": candidate_root,
                            "approval_scope": "PHASE_6_CANDIDATE_EVIDENCE_ROOT_ACCEPTANCE",
                            "status": "APPROVED",
                        }
                    )
                ),
        ...
        "logical_id": "phase6.approval_records",
    ...
    *,
    ...
    return {
        ...
        "coverage_logical_ids_union": sorted(
            ...
            | set(BUNDLE_FILES)
            | {"phase6.candidate_evidence_root"}
        ),
        "disqualification_reason_codes": [],
        "logical_id": "phase6.ai_review_coverage",
...
def build_ai_review_runs(
    ...
    *,
    ...
    adv_run = load_json(adversarial_dir / "phase6.ai_review_run.json")
    int_run = load_json(integrity_dir / "phase6.ai_review_run.json")
    return {
        "candidate_evidence_root": candidate_root,
        "review_runs": sorted(
            [
                {
                    "completed_at": adv_run.get("completed_at", "2026-08-15T23:30:00.000000000Z"),
                    "output_repository_relative_path": repo_relative(
                        adversarial_dir / "phase6.ai_review_output.json"
                    ),
                    ...
                    "run_record_repository_relative_path": repo_relative(
                        adversarial_dir / "phase6.ai_review_run.json"
                    ),
                    "terminal_state": adv_run["terminal_state"],
                },
                {
                    "completed_at": int_run.get("completed_at", "2026-08-15T23:30:00.000000000Z"),
                    "output_repository_relative_path": repo_relative(
                        integrity_dir / "phase6.ai_review_output.json"
                    ),
                    "review_class": integrity["review_class"],
                    "review_output_hash": int_run["review_output_hash"],
                    "review_run_id": int_run["review_run_id"],
                    "run_record_repository_relative_path": repo_relative(
                        integrity_dir / "phase6.ai_review_run.json"
                    ),
...
def build_index_members(bundle: Path, postreview: Path) -> list[dict[str, object]]:
    ...
    postreview_map = {
        "phase6.ai_review_runs": postreview / "phase6.ai_review_runs.json",
        "phase6.ai_review_coverage": postreview / "phase6.ai_review_coverage.json",
        "phase6.approval_records": postreview / "phase6.approval_records.json",
    }
    ...
    candidate_ids = [
        row["logical_id"] for row in members if row["logical_id"] != "phase6.candidate_evidence_root"
    ]
...
def build_acceptance_index(members: list[dict[str, object]], *, candidate_root: str) -> dict[str, object]:
    provisional = {
        ...
        "logical_id": "phase6.acceptance_index",
...
def build_final_result(
    *,
    ...
    provisional = {
        ...
        "logical_id": "phase6.final_acceptance_result",
        "outcome": outcome,
...
def build_all(
    *,
    ...
    write_canonical(
        postreview / "phase6.approval_records.json",
    ...
    write_canonical(postreview / "phase6.ai_review_coverage.json", coverage_doc)
    write_canonical(
        postreview / "phase6.ai_review_runs.json",
    ...
    write_canonical(postreview / "phase6.acceptance_index.json", index_doc)
    ...
    write_canonical(postreview / "phase6.final_acceptance_result.json", final_doc)
    return {
...
def main() -> int:
    ...
    install_guard([])
    run_id, candidate_root, default_bundle = resolve_phase6_bundle()
    ...
    parser.add_argument("--approved-at", default="2026-08-15T23:30:00.000000000Z")
    parser.add_argument("--completed-at", default="2026-08-15T23:32:00.000000000Z")
