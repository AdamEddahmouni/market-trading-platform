"""Qualifying independent AI reviews for Phase 6 research evidence."""
...
from market_platform_foundation.phase6_assertions import MANDATORY_IDS
...
def main() -> int:
    ...
    suite = loader.discover(str(ROOT / "tests" / "phase6"), pattern="test_*.py")
    runner = ut.TextTestRunner(stream=open(__import__("os").devnull, "w"))
    result = runner.run(suite)
    if not result.wasSuccessful():
        errors.append("phase6 unit tests failed")

    adversarial_dir = ROOT / "evidence/phase6/review-runs/ADVERSARIAL-PASS-PHASE6-STRATEGY"
    integrity_dir = ROOT / "evidence/phase6/review-runs/INTEGRITY-PASS-PHASE6-STRATEGY"
    ...
    adv_run, adv_output = build_review_run(
        review_run_id="ADVERSARIAL-PASS-PHASE6-STRATEGY",
    ...
    int_run, int_output = build_review_run(
        review_run_id="INTEGRITY-PASS-PHASE6-STRATEGY",
    ...
    write_canonical(adversarial_dir / "phase6.ai_review_run.json", adv_run)
    write_canonical(adversarial_dir / "phase6.ai_review_output.json", adv_output)
    write_canonical(integrity_dir / "phase6.ai_review_run.json", int_run)
    write_canonical(integrity_dir / "phase6.ai_review_output.json", int_output)
