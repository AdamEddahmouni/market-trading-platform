...
| Professor brief + squeeze SEC | EDGAR whale event vocabulary | `donor_patterns/edgar_whale.py` | Implemented (ingestion unauthorized) |
...
- Whale ingestion, broker adapters, and live/paper execution remain unauthorized per Phase 8 limitations.
- `donor_patterns/*` modules are research/reference implementations until a separate phase authorizes provider ingestion.
...
2. Wire `donor_patterns/edgar_whale.py` to a governed ingestion job (separate authorization).
