...
| O-23 | No point-in-time on OI/earnings/dividends | Leakage risk when live | Whale ledger has cutoff; no earnings join | Backtest leakage | **RESOLVED (fixture scope)** — `BitemporalReferenceStore` + `join_as_of` for OPTIONS_OI / EARNINGS_CALENDAR / DIVIDEND_ASSUMPTION; P0-S1 gate PASS | `runtime/bitemporal_store.py`, `runtime/pit_joins.py` | P0 | P0 |
