...
| MC-D10 | Macro surprise in Futures only | Equity macro events fragmented | `macro_events.py` | Duplicate ontology | Shared macro taxonomy; Futures interprets | F7 + MC11 | Futures+MC | MC11 | **RESOLVED (fixture scope via MC11)** |
...
| MC-D14 | Revision lineage incomplete for news | Backtest knows V3 before V1 | squeeze news ADR 0022 | Temporal leakage | Revision-aware documents MC1 | squeeze news docs | Market Context | MC1 | **RESOLVED (fixture scope)** — BOXL V1/V2 pair + PIT clustering |
| MC-D15 | Social treated as truth in internship | Influence ≠ accuracy | herd_alert, claude_scorer | Reflexive false positives | Separate influence vs accuracy MC14 | influence track | Research | MC14 | **RESOLVED (fixture scope)** — `author_intelligence_v1` |
| MC-D16 | No LLM prompt/model versioning (internship) | Silent semantic drift | claude_scorer | Non-reproducible research | `ModelVersionRef` on all extractions | MC5/MC16 | Market Context | MC16 | **RESOLVED (IMP fixture path)** |
...
| MC-D20 | Same-timestamp Context↔Options risk | Circular reinforcement possible | fusion DAG not enforced for NLP | Self-fulfilling signals | Provenance class + lag rules | cross_lane fusion | Platform | P3 | **RESOLVED (validate_evidence_dag + lag filter)** |
