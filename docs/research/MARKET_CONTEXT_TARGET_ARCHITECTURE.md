...
Current code: **MC1–MC16 IMPLEMENTED** (contracts + fixture entity resolution + event clustering + baseline sentiment + event extraction + expectations/surprise + impact components + catalyst/thesis + attention/diffusion + narrative + macro + reaction + information decay + social/author intelligence + cross-entity propagation + multi-document LLM synthesis). MC16 publishes separate cluster synthesis fields (`theme_agreement_score`, `contradiction_detected`, `consolidated_channels`) with fixture-precomputed LLM labels — no universal news score. MC7/MC8 optionally attach `synthesis_enrichment_v1` metadata from MC16 on matching BOXL clusters without changing fusion scores. Catalyst bridge uses MC8 workspace output; attention uses MC9 `AttentionEvidence` on BOXL fixtures; MC14 keeps influence and accuracy as separate fields; MC15 publishes separate `propagated_*` fields on BOXL/NVDA graph fixtures.
