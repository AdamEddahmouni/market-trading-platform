"""Build Phase 1 postreview gate artifacts for decision acceptance."""
...
from tools.phase1.acceptance_algorithms import (
...
DEFAULT_POSTREVIEW = ROOT / "evidence/phase1/postreview"
DEFAULT_ADVERSARIAL = ROOT / "evidence/phase1/review-runs/ADVERSARIAL-PASS-PHASE1-DECISIONS"
DEFAULT_INTEGRITY = ROOT / "evidence/phase1/review-runs/INTEGRITY-PASS-PHASE1-DECISIONS"
...
def load_review_output(review_dir: Path) -> dict[str, object]:
    return load_json(review_dir / "phase1.ai_review_output.json")
...
def build_approval_records(*, approved_at: str) -> dict[str, object]:
    return {
                    ...
                    canonical_bytes(
                        {
                            ...
                            "approved_logical_id": "phase1.candidate_evidence_root",
                            "approved_sha256": CANDIDATE_ROOT,
                            "approval_scope": "PHASE_1_DECISION_CANDIDATE_EVIDENCE_ROOT_ACCEPTANCE",
                            "status": "APPROVED",
                        }
                    )
                ),
        ...
        "logical_id": "phase1.approval_records",
...
def build_coverage(adversarial: dict[str, object], integrity: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "coverage_logical_ids_union": sorted(
            ...
            | {"phase1.candidate_evidence_root", "phase1.adr_verifier_result"}
        ),
        "disqualification_reason_codes": [],
        "duplicate_identity_results": [],
        "logical_id": "phase1.ai_review_coverage",
...
def build_ai_review_runs(
    ...
    adv_run = load_json(adversarial_dir / "phase1.ai_review_run.json")
    int_run = load_json(integrity_dir / "phase1.ai_review_run.json")
    return {
        "candidate_evidence_root": CANDIDATE_ROOT,
        "review_runs": sorted(
            [
                {
                    "completed_at": adv_run["completed_at"],
                    "output_repository_relative_path": repo_relative(
                        adversarial_dir / "phase1.ai_review_output.json"
                    ),
                    ...
                    "run_record_repository_relative_path": repo_relative(
                        adversarial_dir / "phase1.ai_review_run.json"
                    ),
                    "terminal_state": adv_run["terminal_state"],
                },
                {
                    "completed_at": int_run["completed_at"],
                    "output_repository_relative_path": repo_relative(
                        integrity_dir / "phase1.ai_review_output.json"
                    ),
                    "review_class": integrity["review_class"],
                    "review_output_hash": int_run["review_output_hash"],
                    "review_run_id": int_run["review_run_id"],
                    "run_record_repository_relative_path": repo_relative(
                        integrity_dir / "phase1.ai_review_run.json"
                    ),
...
def build_index_members(bundle: Path, postreview: Path) -> list[dict[str, object]]:
    ...
    postreview_map = {
        "phase1.ai_review_runs": postreview / "phase1.ai_review_runs.json",
        "phase1.ai_review_coverage": postreview / "phase1.ai_review_coverage.json",
        "phase1.approval_records": postreview / "phase1.approval_records.json",
    }
    ...
    candidate_ids = [row["logical_id"] for row in members if row["logical_id"] != "phase1.candidate_evidence_root"]
...
def build_acceptance_index(members: list[dict[str, object]]) -> dict[str, object]:
    provisional = {
        ...
        "logical_id": "phase1.acceptance_index",
...
def build_final_result(
    *,
    ...
    if integrity.get("recommended_candidate_outcome") == "FAIL":
        invalid_statuses.append("FAIL")
    outcome = derive_final_outcome("PASS", invalid_statuses, False)
    provisional = {
        "adr_verifier_status": "PASS",
        "candidate_evidence_root": CANDIDATE_ROOT,
        "completed_at": completed_at,
        "index_sha256": index["index_sha256"],
        "logical_id": "phase1.final_acceptance_result",
        "outcome": outcome,
...
def build_all(
    *,
    ...
    write_canonical(postreview / "phase1.approval_records.json", build_approval_records(approved_at=approved_at))
    coverage_doc = build_coverage(adversarial, integrity)
    write_canonical(postreview / "phase1.ai_review_coverage.json", coverage_doc)
    write_canonical(
        postreview / "phase1.ai_review_runs.json",
    ...
    write_canonical(postreview / "phase1.acceptance_index.json", index_doc)
    ...
    write_canonical(postreview / "phase1.final_acceptance_result.json", final_doc)
    return {
...
def main() -> int:
    ...
    parser.add_argument("--approved-at", default="2026-08-15T14:40:00.000000000Z")
    parser.add_argument("--completed-at", default="2026-08-15T14:42:00.000000000Z")
