"""CLI: Item 9 calibration preflight (read-only; never fits)."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Item 9 calibration preflight (read-only). "
            "Returns READY or BLOCKED. Never fits. ABSENT authorization does not block READY."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output is not None:
        ...
        print(text)
    # Exit 0 for both READY and BLOCKED — BLOCKED is an honest software outcome.
    return 0
