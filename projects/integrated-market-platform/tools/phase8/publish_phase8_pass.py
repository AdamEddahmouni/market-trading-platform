...
POSTREVIEW_DIR = ROOT / "evidence/phase8/postreview-pass"
...
def publish(*, publication_path: Path) -> dict[str, object]:
    final_path = POSTREVIEW_DIR / "phase8.final_acceptance_result.json"
    index_path = POSTREVIEW_DIR / "phase8.acceptance_index.json"
    ...
    if manifest.get("phase8_status") == "PASS":
        raise ValueError("Phase 8 PASS already published")

    authority_at_publication = write_canonical_json(
        AUTHORITY_PATH,
        {**manifest, "phase8_status": "PASS"},
    )
    ...
    publication_doc = {
        "acceptance_index": {
            ...
            "repository_relative_path": "evidence/phase8/postreview-pass/phase8.acceptance_index.json",
            "root_hash": index_doc.get("root_hash"),
            "sha256": sha256_bytes(index_path.read_bytes()),
        },
        "artifact_type": "PHASE_8_PASS_PUBLICATION",
        ...
        "authority_manifest_at_publication": {
            ...
            "phase7_status": manifest.get("phase7_status"),
            "phase8_status": "PASS",
        ...
        "effect": (
            "Publishes Phase 8 PASS for deterministic end-to-end acceptance on the admitted "
            "equity intraday source. ES-session acceptance remains deferred per ADR-DATA-001."
        ),
        "final_acceptance_result": {
            "final_result_id": final_doc.get("final_result_id"),
            "outcome": final_doc.get("outcome"),
            "repository_relative_path": "evidence/phase8/postreview-pass/phase8.final_acceptance_result.json",
            "sha256": sha256_bytes(final_path.read_bytes()),
        },
        "logical_id": "phase8.pass_publication",
        "principal_id": "PROJECT-PRINCIPAL-001",
        "published_at": "2026-08-17T02:00:00.000000000Z",
