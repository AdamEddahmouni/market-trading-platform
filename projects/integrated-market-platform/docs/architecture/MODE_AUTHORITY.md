...
| Observational clock | If `LIVE_OBSERVATIONAL` and no live quote/event server receive time, `as_of_time` is `UNAVAILABLE` — never the fixture replay cursor — on context, `store.as_of_time()` (same as `display_as_of_time()`), and inspect EVIDENCE. Fixture/replay attention only on `replay_shelf` with `replay_shelf_label=DEMO_REPLAY`, never current RTH `items`. Ranked reads: `EMPTY`, `UNREADY` (+ optional `withheld_ranked_count`), or `READY` per DATA_CONTRACTS feed honesty. **Broker** mutations (orders, Paper submit) remain blocked (`LIVE_OBSERVATIONAL_NO_BROKER_EXECUTION`). **Observational operator lifecycle** WATCH/DISMISS/review ack is permitted: `ExecutionDecisionTraceV1` + `TradeReviewV1` + durable operator acks, no Live execution. **FTEP `--live-ingress` (software, CLI not a daemon):** Finviz fetch (#207) then **HTTP POST** `POST /intelligence/ingest/news` into the **serving** UI API process (`IMP_UI_API_BASE_URL`, default `127.0.0.1:8766`). Ranked Opportunity Engine rows on that handler store use the serving `IntelligenceRepository`: **local_state SQLite** (same database as operator acks / trade reviews / traces) when persist is on; otherwise process-local `INTENTIONAL_EPHEMERAL` memory. Mongo is not the serving composition. This is intermediate software proof, not empirical RTH unattended ingress. Manual operator POST of already-fetched JSON uses the same route; not BUILD 09 `NEWS_EVENT`. |
