...
`error_category` is additive. Demo mutation (`DEMO_MUTATIONS_PROHIBITED`) and Live observational opportunity **mutation** (`LIVE_OBSERVATIONAL_NO_OPPORTUNITY_ENGINE`) are `MODE_BLOCKED`, not `INTERNAL_ERROR`. That reason is the ACK/WATCH/DISMISS fail-close only. Live observational **reads** of `/opportunities/summary` go through `build_ranked_rows` after fixture attention is quarantined and may be `EMPTY` or `READY` from the intelligence repository. P12 (`5f965f32`): the 2026-09-15 `UNAVAILABLE` reason was the request-path `_is_live` early return, not missing EventV1 admission. Mixed discovery is INVESTIGATE / `execution_authority=NONE`, not the Opportunity Engine.

`LIVE_OBSERVATIONAL` `as_of_time` is `UNAVAILABLE` when no live quote or admitted event receive time exists. Optional `as_of_provenance` (`LIVE_RECEIVE` / `UNAVAILABLE`) is additive. Attention `replay_shelf` + `replay_shelf_label=DEMO_REPLAY` is additive; current `items` must not present fixture `att-replay-context` / MC9 / ES cards as RTH.
