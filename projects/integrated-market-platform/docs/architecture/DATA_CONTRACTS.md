...
`error_category` is additive. Demo mutation (`DEMO_MUTATIONS_PROHIBITED`) and Live observational opportunity **mutation** (`LIVE_OBSERVATIONAL_NO_OPPORTUNITY_ENGINE`) are `MODE_BLOCKED`, not `INTERNAL_ERROR`. That reason is the ACK/WATCH/DISMISS fail-close only. Live observational **reads** of `/opportunities/summary` go through `build_ranked_rows` after fixture attention is quarantined and may be `EMPTY` from a zero-qualifying repository. `READY` requires a live receive clock — never when `as_of_time` is `UNAVAILABLE`. INELIGIBLE rows are not ranked on the current book. P12 (`5f965f32`): the 2026-09-15 `UNAVAILABLE` reason was the request-path `_is_live` early return, not missing EventV1 admission. `UiApiHandler` `POST /intelligence/ingest/news` admits already-fetched Finviz/news rows as EventV1 through `ObservationIngressRouter` (`put_event`). Zero-qualifying news stays `EMPTY`; qualifying observational news may mint `OpportunityV1` without Live authority. Mixed discovery is INVESTIGATE / `execution_authority=NONE`, not the Opportunity Engine.
