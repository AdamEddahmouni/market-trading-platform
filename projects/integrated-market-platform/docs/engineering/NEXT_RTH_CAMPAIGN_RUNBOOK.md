...
7. After future **software** merges, update **CANDIDATE_RUNTIME** / freeze state here and pin [PROGRAM_STATUS.md](../platform/PROGRAM_STATUS.md) separately. This runbook tracks operator flow.
...
| 11 | Job-kill / terminal / breakaway survival | **PARTIAL** | **UNPROVEN** on this host; fail-visible stale/dead detection is the guarantee |
| 12 | OpenD preflight path | **PARTIAL** | Readiness CLIs on tip; live OpenD reachability **Operator at arm** (not probed here) |
...
**Freeze decision:** **NO.** Any remaining **FAIL** or unpaid **PARTIAL** operator-at-arm gates block freeze. Known durable **PARTIAL:** gate 11 (job/terminal detachment).
