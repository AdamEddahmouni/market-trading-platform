...
| **Summary** | Added read-only Item 7 harness that classifies vendor `get_market_snapshot` bid/ask without synthesizing from `last_price`, under distinct capability `SNAPSHOT_BBO` (not `US_EQUITY_L1`). Fixture tests cover missing/invalid spread/stale/delayed/valid BBO, temporal order, identity, and entitlement failure; live CLI probes only when US RTH and loopback OpenD are available, else honest block. Outcomes are `REAL_SNAPSHOT_BBO_VALIDATED` or `DERIVED_BBO_DESIGN_REQUIRED` only — never `ITEM7_COMPLETE`. |
| **Key files** | `tools/moomoo/item7_bbo_snapshot.py`, `tools/item7_bbo_snapshot_probe.py`, `tests/providers/test_item7_bbo_snapshot.py` |
...
| **Notes** | Did not touch opend_capture_ledger, FTEP, paper/calibration, UI, or L1 adapter semantics. No orders. |
