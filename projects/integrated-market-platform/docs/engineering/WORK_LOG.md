...
| **Tests** | Focused: `PYTHONPATH=src python3 -m unittest tests.research.test_research_export_v1 tests.research.test_wave1_experiment_harness tests.research.test_matlab_environment -v` → **24 passed**. FAST: `python3 tools/imp.py validate fast` → **23 passed**. Affected vs `origin/main`: **927 passed**, 0 skipped, 0 failures, 0 errors (`core_checkpoint_required=true` from `validation_manifest.json` + unclassified artifact path; not a FULL closure). Docs: `python3 tools/check_docs_links.py` → 189 files OK. |
| **Related** | [RESEARCH_EXPORT_V1.md](../research/RESEARCH_EXPORT_V1.md), QR-01/QR-03 MATLAB lab plan, Wave 1 `export_gate.py` |
| **Notes** | Did not stamp `PIT-PASS`. Did not run Wave 1 OOS. No hop `1381619`, no FTEP session, Live off. MATLAB not installed on cloud. |
