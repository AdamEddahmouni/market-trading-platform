...
| **Tests** | `PYTHONPATH=src python3 -m unittest tests.ui1.test_opportunity_api tests.intelligence.test_opportunity_ingest -v` → **18 passed**; related OE + `test_error_taxonomy` **36 passed**; `python3 tools/imp.py format` + `lint` + `validate fast` → **23 passed**; `python3 tools/imp.py validate changed --paths-file` (origin/main...HEAD) → **2481 passed / 36 skipped**; `python3 tools/check_docs_links.py` → OK 189 files. GitHub IMP Validation 9/9 including `validate-ui` pending on this PR. |
