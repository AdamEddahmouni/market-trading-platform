...
| **Tests** | `PYTHONPATH=src python3 -m unittest tests.providers.test_finviz_elite_context tests.providers.test_providers.ProviderContractTests` — **15 passed**. `python3 tools/imp.py test focused` (13 Finviz overlay selectors) **passed**. `python3 tools/check_docs_links.py` **OK** (188 files). `python3 tools/imp.py lint` **passed**. `PYTHONPATH=src python3 tools/validate.py changed --paths-file` vs `origin/main` — **656 tests, 11 skipped, 0 fail / 0 err**. |
