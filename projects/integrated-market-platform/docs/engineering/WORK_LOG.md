...
| **Tests** | Focused durable-loop + news ingest/PIT + trade-review/schema + trace runtime: **37 passed**, 0 failed. v2→v9 migration **1 passed**. `validate changed` to follow. |
...
| **Status** | `complete` |
| **Area** | `ui/radar`, `ui/opportunity` |
| **Summary** | Stopped labeling IMP headline language as an observed fact on Radar detail: moved "Surfaced headline" into derived epistemic layers, aligned the operator brief "What happened?" row with queue-scan honesty, renamed residual "Market behavior" to "Headline", and surfaced attached `expires_at`/`expiry` in invalidation lines when present. |
| **Key files** | `ui/src/components/opportunity/opportunityEpistemicLayers.ts` (+test); `opportunityOperatorBrief.ts` (+test); `opportunityDetailModel.ts` |
| **Tests** | `npx vitest run --environment node` on `opportunityEpistemicLayers.test.ts`, `opportunityOperatorBrief.test.ts`, `opportunityDetailModel.test.ts` — **32 passed** |
| **Related** | Operator comprehension lane (post-RTH optional); does not merge Live or touch Item 9 |
| **Notes** | Remaining high-harm gaps (Radar risk UNAVAILABLE stubs; Discover→Opportunity provenance bridge; opportunity↔Paper evidence link) need backend/data or larger feature work — not in this PR |
...
| **Status** | `complete` |
| **Area** | `docs/platform` |
| **Summary** | Absorbed `origin/main` **`9df351d8`** (merge [#358](https://github.com/AdamEddahmouni/market-trading-platform/pull/358) UNAVAILABLE eligibility workspace/ack gate) into [#357](https://github.com/AdamEddahmouni/market-trading-platform/pull/357) and retargeted PROGRAM_STATUS **CURRENT_MAIN** / **CURRENT_SOFTWARE_IMPLEMENTATION** (next-action and Lane F closeout `CURRENT_SOFTWARE_IMPLEMENTATION_SHA`) to that software tip. Pin is the **#358 merge commit**, not unique HEAD `9bc9dd62`. Item 9 **2/3 IDLE**, **NOT_CALIBRATED**, `ITEM9_CALIBRATION_RUN` **FORBIDDEN**, collector **`fed2d9f7`** unchanged, epoch `121031` **not** backfilled, weekend **no** collection, Live **OFF**, Full30 **NOT_RUN**, #222 **HOLD**. **Not** a collector retarget. Did **not** merge #357 or #222. |
| **Key files** | `docs/platform/PROGRAM_STATUS.md`; `docs/engineering/WORK_LOG.md` |
| **Tests** | `python tools/check_docs_links.py` (docs-only) |
| **Related** | [#350](https://github.com/AdamEddahmouni/market-trading-platform/pull/350) pin convention; [#357](https://github.com/AdamEddahmouni/market-trading-platform/pull/357) retarget after [#358](https://github.com/AdamEddahmouni/market-trading-platform/pull/358) on `main` @ `9df351d8` |
| **Notes** | Isolated worktree `.worktrees/weekend-current-main-pin-356` on `docs/weekend-current-main-pin-356`. Absorbed `origin/main` with merge (no rebase/force). Prior MERGE-CLEAR on `9d4a957d` is **STALE** — #357 needs independent re-review. Primary desktop `44b8673e` untouched. |
...
| **Status** | `complete` |
| **Area** | `docs/platform` |
| **Summary** | Absorbed `origin/main` **`2ce1d9d1`** (merge [#347](https://github.com/AdamEddahmouni/market-trading-platform/pull/347) Radar leftover Demo discovery-desk copy) into [#345](https://github.com/AdamEddahmouni/market-trading-platform/pull/345) and retargeted PROGRAM_STATUS **CURRENT_MAIN** / **CURRENT_SOFTWARE_IMPLEMENTATION** (and next-action / closeout software SHA) to that software tip. Prior absorb of **`1aff8906`** ([#346](https://github.com/AdamEddahmouni/market-trading-platform/pull/346)) was already on this branch — did **not** double-merge that sibling. Item 9 **2/3 IDLE**, collector **`fed2d9f7`**, **NOT CALIBRATED**, `ITEM9_CALIBRATION_RUN` **FORBIDDEN**, Live **OFF**, #222 isolated unchanged. Did **not** merge #222/#314. |
| **Key files** | `docs/platform/PROGRAM_STATUS.md` |
| **Tests** | Docs SHA pin only — CI on push |
| **Related** | [#345](https://github.com/AdamEddahmouni/market-trading-platform/pull/345) prior pin (`1aff8906`); [#347](https://github.com/AdamEddahmouni/market-trading-platform/pull/347); [#346](https://github.com/AdamEddahmouni/market-trading-platform/pull/346) |
| **Notes** | Isolated worktree `.worktrees/weekend-pin-current-main-344`. **ITEM9_FROZEN_COLLECTOR** untouched. Primary desktop `44b8673e` untouched. WORK_LOG keep-both (no conflict; #347 did not edit WORK_LOG). |
...
| **Status** | `complete` |
| **Area** | `docs/platform` |
| **Summary** | Retargeted PROGRAM_STATUS **CURRENT_MAIN** / **CURRENT_SOFTWARE_IMPLEMENTATION** (and next-action / closeout software SHA) to `origin/main` **`a4700da1`** (merge [#342](https://github.com/AdamEddahmouni/market-trading-platform/pull/342) leftover mkdtemp unit-test hermeticity; ancestry includes [#341](https://github.com/AdamEddahmouni/market-trading-platform/pull/341) docs pin on `ce8cd20f` plus [#340](https://github.com/AdamEddahmouni/market-trading-platform/pull/340)). Item 9 **2/3 IDLE**, collector **`fed2d9f7`**, **NOT CALIBRATED**, `ITEM9_CALIBRATION_RUN` **FORBIDDEN**, Live **OFF**, #222 isolated unchanged. Did **not** merge #222. |
| **Key files** | `docs/platform/PROGRAM_STATUS.md` |
| **Tests** | Docs SHA pin only — CI on push |
| **Related** | [#341](https://github.com/AdamEddahmouni/market-trading-platform/pull/341) prior pin (`ce8cd20f`); [#342](https://github.com/AdamEddahmouni/market-trading-platform/pull/342); [#340](https://github.com/AdamEddahmouni/market-trading-platform/pull/340) |
| **Notes** | Isolated worktree `.worktrees/weekend-pin-current-main-342` from `origin/main` @ `a4700da1`. **ITEM9_FROZEN_COLLECTOR** untouched. Primary desktop `44b8673e` untouched. |

## 2026-09-19 — Pin CURRENT_MAIN to origin/main `ce8cd20f` after #340

| Field | Value |
|-------|-------|
| **Status** | `complete` |
| **Area** | `docs/platform` |
| **Summary** | Retargeted PROGRAM_STATUS **CURRENT_MAIN** / **CURRENT_SOFTWARE_IMPLEMENTATION** (and next-action / closeout software SHA) to `origin/main` **`ce8cd20f`** (merge [#340](https://github.com/AdamEddahmouni/market-trading-platform/pull/340) Weekend Control live-clock UNREADY off the repair list; ancestry includes [#339](https://github.com/AdamEddahmouni/market-trading-platform/pull/339) docs pin on `a2914ad2` plus [#338](https://github.com/AdamEddahmouni/market-trading-platform/pull/338)). Item 9 **2/3 IDLE**, collector **`fed2d9f7`**, **NOT CALIBRATED**, `ITEM9_CALIBRATION_RUN` **FORBIDDEN**, Live **OFF**, #222 isolated unchanged. Did **not** merge #222. |
| **Key files** | `docs/platform/PROGRAM_STATUS.md` |
| **Tests** | Docs SHA pin only — CI on push |
| **Related** | [#339](https://github.com/AdamEddahmouni/market-trading-platform/pull/339) prior pin (`a2914ad2`); [#340](https://github.com/AdamEddahmouni/market-trading-platform/pull/340); [#338](https://github.com/AdamEddahmouni/market-trading-platform/pull/338) |
| **Notes** | Isolated worktree `.worktrees/weekend-pin-current-main-340` from `origin/main` @ `ce8cd20f`. **ITEM9_FROZEN_COLLECTOR** untouched. Primary desktop `44b8673e` untouched. |
