...
| **Tests** | `tests.intelligence.test_path_a_forecast_producer` **16 passed**. Path A producer + store + prereg + prospective + catalog + scan caller **98 passed / 0 fail**. `python3 tools/validate.py changed --paths-file` (12 paths vs `#51` `65e19cd`) **2563 passed / 25 skipped / 0 fail / 0 err** (`core_checkpoint_required=true`). `python3 tools/check_docs_links.py` **OK (188 files)**. Live still `LIVE_SCAN_CALLER_FORBIDDEN`. |
...
| **Related** | Stacks on #49 `f920ef5` (ForecastV1 load). Item 7 of the provider activation program. |
| **Notes** | A fused software artifact is not empirical. Default production fusion still has no live specialist contributors on this hop, so the operator path fail-closes without them. Did not touch OpenD transport / #50. Did not declare FTEP `EMPIRICAL_ACTIVE` or Live. Frozen FTEP-V1-001 JSON not touched. |
...
| **Related** | PR #42 (`91b8889` persist hop, `44fdcec`/`33e6705` CLI scan-caller invoke, `df2b66c` real baseline strategy catalog). Item 8 of the activation program (canonical hop → optional PD-09 v6 persistence). |
| **Notes** | Real reachability still requires an operator-supplied, already-frozen/activated Forward-Test session (out-of-band, e.g. via `tools/ftep_session_start.py`); the CLI does not fabricate or freeze a campaign, matching the doctrine's "no campaign activation from documentation alone." No MATCHED fixture is minted by default (`build_paper_demo_path_a_invoke` honesty invoke is untouched); MATCHED-fixture cases only exist inside the new tests. FTEP is not `EMPIRICAL_ACTIVE`/`CALIBRATED`. No secrets printed — `persist_context_injected` is a boolean; session/account/strategy identifiers are operator-supplied CLI args, not credentials. Rebased onto PR #42 tip (`df2b66c`, real baseline strategy catalog) after that PR's catalog wiring landed on the shared base branch; both the catalog and this persist-context inject coexist (disjoint code paths — catalog lives in `strategy/path_a_prospective.py`/`path_a_strategy_catalog.py`, persist inject lives in `tools/path_a_prospective_run.py`). |
