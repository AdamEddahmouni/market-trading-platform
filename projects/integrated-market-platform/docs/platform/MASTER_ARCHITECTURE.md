...
| Candidate opportunities | Radar Screeners / Discover mixed (`INVESTIGATE`, `execution_authority=NONE`) | Candidates are investigation-only. They are **not** Opportunity Engine mint. Weekend copy already labels this. Leftover donor catalyst overlay on Research screens no longer presents as an internship demo or live feed ([#361](https://github.com/AdamEddahmouni/market-trading-platform/pull/361)); historical `internship-project-main` remains provenance-only. Remaining **UI comprehension**: investigation-only vs OE mint. |
...
| Evidence / outcomes | Research evidence, OF-01 ledger, FTEP/Item 9 receipts, prediction settlement | Evidence classes stay distinct. Prospective Item 9 collection is frozen at collector `fed2d9f7`, not current software. Operator SHA tables should match fetched `origin/main` (this pin: **CURRENT_MAIN** / **CURRENT_SOFTWARE_IMPLEMENTATION** `5b74876d` after [#380](https://github.com/AdamEddahmouni/market-trading-platform/pull/380); historical pin `d1c50455` after [#361](https://github.com/AdamEddahmouni/market-trading-platform/pull/361) is **not** the current tip). **docs** if later merges leave those tables lagging. |
