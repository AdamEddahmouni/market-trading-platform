...
class StatePathDiagnosticTests(unittest.TestCase):
    def test_effective_state_dir_honors_imp_state_dir(self) -> None:
        with tempfile.TemporaryDirectory() as imp_tmp, tempfile.TemporaryDirectory() as override_tmp:
            imp_root = Path(imp_tmp)
            override = Path(override_tmp)
    ...
    def test_worktree_mismatch_warning_when_canonical_has_sessions(self) -> None:
        with (
            tempfile.TemporaryDirectory() as imp_tmp,
            tempfile.TemporaryDirectory() as canonical_tmp,
            tempfile.TemporaryDirectory() as effective_tmp,
        ):
            imp_root = Path(imp_tmp)
            canonical = Path(canonical_tmp)
            effective = Path(effective_tmp)
