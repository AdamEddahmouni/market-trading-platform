...
class RankedSummaryLeakAuditTests(unittest.TestCase):
    ...
    def test_watch_does_not_place_a_broker_or_paper_order(self) -> None:
        status, ingest = _http_json(self.port, "POST", NEWS_INGEST_ROUTE, body=_qualifying_news_body())
        self.assertEqual(status, 200)
        self.assertIsInstance(ingest, dict)
        opportunity_id = str((ingest.get("opportunity_ids") or [None])[0])
        ...
        self.assertNotEqual(watch_status, 500)
        self.assertNotIn("order_id", str(watch_body))
        if watch_status == 200:
            self.assertIsInstance(watch_body, dict)
            self.assertNotIn("order_id", watch_body)
        else:
            self.assertIn("LIVE_OBSERVATIONAL", str(watch_body))
