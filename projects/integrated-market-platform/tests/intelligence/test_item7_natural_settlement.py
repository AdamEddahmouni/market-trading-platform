...
def _seed_snapshot_jsonl(jsonl: Path) -> None:
    jsonl.write_text(
        json.dumps(
...
def _outcome_record_types(jsonl: Path) -> set[str]:
    return {
...
def _outcome_payloads(jsonl: Path) -> list[dict]:
    rows = [
        json.loads(line)
        for line in jsonl.read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    return [row["payload"] for row in rows if row.get("record_type") == "outcome"]
...
class Item7NaturalSettlementTests(unittest.TestCase):
    ...
    def test_before_maturity_reports_not_due_without_outcome(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.natural_settlement["due_unlabelable"], 0)
            self.assertNotIn("outcome", _outcome_record_types(jsonl))

    def test_at_maturity_without_terminal_trade_reports_unlabelable_without_persist(self) -> None:
        self._seed_contributor()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            os.environ["IMP_STATE_DIR"] = str(root)
            ...
            self.assertEqual(result.natural_settlement["outcomes_appended"], 0)
            reasons = result.natural_settlement["refusal_reasons"]
            self.assertIn(UnlabelableReason.NO_TARGET_OBSERVATION.value, reasons)
            self.assertNotIn("outcome", _outcome_record_types(jsonl))
            repo, _report = load_governed_intelligence_repository(persistence_root=root)
            stores = getattr(repo, "_stores", {})
            outcomes = stores.get("outcomes") if isinstance(stores, dict) else None
            self.assertFalse(outcomes)

    def test_maturity_unlabelable_leaves_ledger_open_for_later_terminal(self) -> None:
        """Regression: due UNLABELABLE must not seal; later terminal can SETTLED."""
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(first.natural_settlement["due_unlabelable"], 1)
            self.assertEqual(first.natural_settlement["settled"], 0)
            self.assertEqual(first.natural_settlement["outcomes_appended"], 0)
            self.assertNotIn("outcome", _outcome_record_types(jsonl))
            repo_after_first, _ = load_governed_intelligence_repository(persistence_root=root)
            stores = getattr(repo_after_first, "_stores", {})
            outcomes = stores.get("outcomes") if isinstance(stores, dict) else None
            self.assertFalse(outcomes)
            ...
            self.assertEqual(second.natural_settlement["due_unlabelable"], 0)
            self.assertEqual(second.natural_settlement["outcomes_appended"], 1)
            payloads = _outcome_payloads(jsonl)
            self.assertEqual(len(payloads), 1)
            self.assertEqual(
                payloads[0]["resolution_status"],
                OutcomeResolutionStatus.SETTLED.value,
            )
    ...
    def test_late_terminal_append_after_not_due_settles_pending_ledger(self) -> None:
        self._seed_contributor()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            os.environ["IMP_STATE_DIR"] = str(root)
