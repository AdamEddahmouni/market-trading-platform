...
class FtepProspectiveCatalystIngressTests(unittest.TestCase):
    ...
    def test_live_ingress_fail_closed_under_fixture_smoke(self) -> None:
        ...
        self.assertEqual(
            payload["prospective_ingress"]["classification"],
            CLASS_SESSION_UNAVAILABLE,
        )
        self.assertEqual(payload["ingress_outcome"], "LIVE_INGRESS_UNAVAILABLE")
        self.assertEqual(payload["ingress_classification"], CLASS_SESSION_UNAVAILABLE)

    def test_timeout_classified_without_retry(self) -> None:
        client = _StubFinvizClient(success=False, error="REQUEST_TIMEOUT")
        result = collect_finviz_prospective_attention_rows(
            REPO_ROOT,
            "FTEP-V1-002",
            live_ingress=True,
        ...
        self.assertEqual(result.classification, CLASS_TIMEOUT)
        self.assertEqual(client.fetch_calls, 1)

    def test_malformed_fetch_payload_classified(self) -> None:
        class _BadClient:
            fetch_calls = 0

            def fetch_news(self, *, force: bool = False) -> object:
                del force
                self.fetch_calls = 1
                return ["not", "a", "dict"]
        ...
        self.assertEqual(result.classification, CLASS_MALFORMED_RESPONSE)
