...
vi.mock("./api/hooks", () => ({
  ...
  useInstrumentSelectorQuery: () => ({ data: { results: [] }, isLoading: false }),
  ...
  useOptionsProductQuery: () => ({
    ...
    data: {
      ...
      runtime: { instrument_kind: "OPTION_CONTRACT", quantity_unit: "CONTRACTS" },
      orders: [],
    },
  }),
  useFuturesProductQuery: () => ({
    isLoading: false,
    data: {
      schema_version: "1",
      domain: "futures",
      ...
      status: "AVAILABLE",
      execution_available: true,
      runtime: { instrument_kind: "FUTURE_CONTRACT", quantity_unit: "CONTRACTS" },
      orders: [],
      exposure: { margin: { state: "AVAILABLE" } },
    },
  }),
...
describe("App mode launcher integration", () => {
  ...
  it("does not toggle the assistant when A is pressed in a select", async () => {
    ...
    expect(screen.queryByRole("complementary", { name: "AI research assistant" })).not.toBeInTheDocument();
    fireEvent.keyDown(window, { key: "a" });
    expect(await screen.findByRole("complementary", { name: "AI research assistant" })).toBeInTheDocument();
    select.remove();
  });
