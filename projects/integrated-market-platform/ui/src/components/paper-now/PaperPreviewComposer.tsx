...
export function PaperPreviewComposer({ instrumentId, side, quantityText, maxOrderShares, disabledReason, pending, error, preview, canOpenWorkspace, onSideChange, onQuantityChange, onPreview, onOpenWorkspace }: Props) {
  ...
  return (
    <section className="paper-panel paper-preview-panel" aria-label="Order preview">
      ...
      <p>
        Choose direction and size. Optional preview checks risk here; authoritative preview and submit happen in the
      ...
      <div className="paper-preview-actions">
        ...
        {canOpenWorkspace ? (
          <button type="button" onClick={onOpenWorkspace}>Open workspace and revalidate</button>
        ) : null}
      </div>
