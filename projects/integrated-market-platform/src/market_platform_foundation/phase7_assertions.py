"""Phase 7 risk, simulation, and accounting assertion registry and evaluator."""
...
MANDATORY_IDS = ("EXE-001", "EXE-002", "EXE-003", "SAFE-003")

_ROLES = {
    "EXE-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "EXE-002": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "EXE-003": (["ARCHITECTURE_LEAD"], ["RELEASE_OWNER"], ["INDEPENDENT_REVIEWER"]),
...
def build_registry(path: Path) -> dict[str, object]:
    ...
    if not isinstance(raw, dict) or raw.get("registry_version") != "1.0.0":
        raise ValueError("unsupported phase7 assertion registry")
