"""Phase 8 end-to-end acceptance assertion registry and evaluator."""
...
MANDATORY_IDS = ("AE-001", "DET-001", "ROLLUP-001", "SAFE-003")

_ROLES = {
    "AE-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER", "RELEASE_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "DET-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "ROLLUP-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
...
def build_registry(path: Path) -> dict[str, object]:
    ...
    if not isinstance(raw, dict) or raw.get("registry_version") != "1.0.0":
        raise ValueError("unsupported phase8 assertion registry")
