"""Phase 6 preregistered strategy assertion registry and evaluator."""
...
MANDATORY_IDS = ("STRAT-001", "ABST-001", "PIT-STRAT-001", "DET-001", "SAFE-003")

_ROLES = {
    "STRAT-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "ABST-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "PIT-STRAT-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
...
def build_registry(path: Path) -> dict[str, object]:
    ...
    if not isinstance(raw, dict) or raw.get("registry_version") != "1.0.0":
        raise ValueError("unsupported phase6 assertion registry")
