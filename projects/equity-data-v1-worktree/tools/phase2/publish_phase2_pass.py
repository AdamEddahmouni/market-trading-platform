...
POSTREVIEW_DIR = ROOT / "evidence/phase2/postreview-pass"
PUBLICATION_PATH = ROOT / "docs/superpowers/governance/2026-08-15-phase-2-pass-publication.json"
...
def publish(*, publication_path: Path) -> dict[str, object]:
    final_path = POSTREVIEW_DIR / "phase2.final_acceptance_result.json"
    index_path = POSTREVIEW_DIR / "phase2.acceptance_index.json"
    ...
    if manifest.get("phase2_status") == "PASS":
        raise ValueError("Phase 2 PASS already published")

    authority_at_publication = write_canonical_json(
        AUTHORITY_PATH,
        {**manifest, "phase2_status": "PASS"},
    )
    ...
    publication_doc = {
        "acceptance_index": {
            ...
            "repository_relative_path": "evidence/phase2/postreview-pass/phase2.acceptance_index.json",
            "root_hash": index_doc.get("root_hash"),
            "sha256": sha256_bytes(index_path.read_bytes()),
        },
        "artifact_type": "PHASE_2_PASS_PUBLICATION",
        ...
        "effect": (
            "Publishes Phase 2 PASS for canonical contracts and deterministic replay "
            "on synthetic fixtures. Phase 3 adapter work remains unauthorized."
        ),
        "final_acceptance_result": {
            "final_result_id": final_doc.get("final_result_id"),
            "outcome": final_doc.get("outcome"),
            "repository_relative_path": "evidence/phase2/postreview-pass/phase2.final_acceptance_result.json",
            "sha256": sha256_bytes(final_path.read_bytes()),
        },
        "logical_id": "phase2.pass_publication",
    ...
    roadmap = roadmap.replace(
        "| Phase 2 — canonical contracts and replay | Establish identity, availability, revision, quality, and deterministic replay contracts before model work. | draft spec/plan prepared; **unauthorized** pending principal approval |",
