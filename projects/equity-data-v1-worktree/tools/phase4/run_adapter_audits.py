"""Qualifying independent AI reviews for Phase 3 adapter evidence."""
...
from market_platform_foundation.phase3_assertions import MANDATORY_IDS
...
def main() -> int:
    ...
    suite = loader.discover(str(ROOT / "tests" / "phase3"), pattern="test_*.py")
    runner = ut.TextTestRunner(stream=open(__import__("os").devnull, "w"))
    result = runner.run(suite)
    if not result.wasSuccessful():
        errors.append("phase3 unit tests failed")

    adversarial_dir = ROOT / "evidence/phase3/review-runs/ADVERSARIAL-PASS-PHASE3-ADAPTER"
    integrity_dir = ROOT / "evidence/phase3/review-runs/INTEGRITY-PASS-PHASE3-ADAPTER"
    ...
    adv_run, adv_output = build_review_run(
        review_run_id="ADVERSARIAL-PASS-PHASE3-ADAPTER",
    ...
    int_run, int_output = build_review_run(
        review_run_id="INTEGRITY-PASS-PHASE3-ADAPTER",
        review_class="INTEGRITY_AND_REPRODUCTION_AUDIT",
        refs=refs,
        errors=errors,
    )
    int_run["completed_at"] = now
    write_canonical(adversarial_dir / "phase3.ai_review_run.json", adv_run)
    write_canonical(adversarial_dir / "phase3.ai_review_output.json", adv_output)
    write_canonical(integrity_dir / "phase3.ai_review_run.json", int_run)
    write_canonical(integrity_dir / "phase3.ai_review_output.json", int_output)
