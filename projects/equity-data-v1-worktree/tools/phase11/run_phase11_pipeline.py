"""Build Phase 11 options whale family acceptance evidence."""
...
REGISTRY_PATH = ROOT / "manifests/phase11/assertion-predicates.json"
FIXTURE_MANIFEST = ROOT / "tests/fixtures/providers/options/admission_manifest.json"
EVALUATED_AT = "2026-08-17T18:00:00.000000000Z"
...
from market_platform_foundation.phase11_assertions import aggregate_status, build_registry, create_run_manifest, evaluate_run
...
from market_platform_foundation.providers.whale_ledger import WHALE_ENTITLED_OPTIONS, build_combined_fixture_ledger
...
def _safe003_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_11_SAFE003_REPORT",
        "logical_id": "phase11.safe003_report",
...
def _fixture_report() -> dict[str, object]:
    ...
    if not isinstance(claims, list):
        ...
        for claim in ("unusual_volume", "direction_ambiguity"):
            if claim not in claims:
                failures.append(f"MISSING_{claim}")
    return {
        "artifact_type": "PHASE_11_FIXTURE_REPORT",
        "failures": failures,
        "logical_id": "phase11.fixture_report",
        "status": "PASS" if not failures else "FAIL",
    }


def _options_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_11_OPTIONS_REPORT",
        "deterministic_envelope_ids": deterministic,
        "event_count": len(first_ids),
        "logical_id": "phase11.options_report",
...
def _pit_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_11_PIT_REPORT",
        "event_count": len(result.events),
        "logical_id": "phase11.pit_report",
...
def _whale_report() -> dict[str, object]:
    ...
    failures: list[str] = []
    if biya.get("status") != "available" or biya.get("reason_code") != WHALE_ENTITLED_OPTIONS:
        failures.append("BIYA_UNAVAILABLE")
    ...
    return {
        "artifact_type": "PHASE_11_WHALE_REPORT",
        "failures": failures,
        "logical_id": "phase11.whale_report",
        "status": "PASS" if not failures else "FAIL",
    }
...
def _ui_report() -> dict[str, object]:
    ...
    failures: list[str] = []
    if by_id.get("whale.options", {}).get("state") != "AVAILABLE":
    ...
    return {
        "artifact_type": "PHASE_11_UI_REPORT",
        "failures": failures,
        "logical_id": "phase11.ui_report",
        "status": "PASS" if not failures else "FAIL",
    }
...
def build_evidence(output_dir: Path) -> dict[str, object]:
    ...
    write_canonical_json(output_dir / "options-report.json", options)
    ...
    observations = {
        "P11-FIX-001": {"failures": fixture.get("failures"), "reason_codes": fixture.get("failures", []), "status": fixture.get("status")},
        "P11-OPT-001": {"reason_codes": [] if options.get("status") == "PASS" else ["P11-OPT-001-FAIL"], "status": options.get("status")},
        "P11-PIT-001": {"reason_codes": [] if pit.get("status") == "PASS" else ["P11-PIT-001-FAIL"], "status": pit.get("status")},
        "P11-WHALE-001": {"failures": whale.get("failures"), "reason_codes": whale.get("failures", []), "status": whale.get("status")},
        "P11-UI-001": {"failures": ui.get("failures"), "reason_codes": ui.get("failures", []), "status": ui.get("status")},
        "SAFE-003": {"reason_codes": safe003.get("reason_codes"), "status": safe003.get("status")},
    }

    members = {
        "phase11.fixture_report": "fixture-report.json",
        "phase11.options_report": "options-report.json",
        "phase11.pit_report": "pit-report.json",
        "phase11.safe003_report": "safe003-report.json",
        "phase11.ui_report": "ui-report.json",
        "phase11.whale_report": "whale-report.json",
    }
    selected_evidence = []
    ...
    manifest_inputs = {
        ...
        "subject_manifest_hash": sha256_bytes(canonical_bytes({"track": "phase11", "root_id": "ROOT-2E7C91F4"})),
        "tool_versions": ["phase11.run_phase11_pipeline/1.0.0"],
    }
    ...
    bundle_members = {
        **members,
        "phase11.assertion_aggregate": "assertion-aggregate.json",
        "phase11.assertion_registry": "assertion_registry.json",
        "phase11.assertion_results": "assertion-results.json",
        "phase11.assertion_run_manifest": "assertion-run-manifest.json",
    }
    candidate_members = []
    ...
    candidate_body = {
        ...
        "logical_id": "phase11.candidate_evidence_root",
        "run_id": run_id,
    }
