"""Build Phase 5R postreview gate artifacts."""
...
from tools.phase5r.acceptance_algorithms import (
...
DEFAULT_POSTREVIEW = ROOT / "evidence/phase5r/postreview-pass"
DEFAULT_ADVERSARIAL = ROOT / "evidence/phase5r/review-runs/ADVERSARIAL-PASS-PHASE5R-RESEARCH"
DEFAULT_INTEGRITY = ROOT / "evidence/phase5r/review-runs/INTEGRITY-PASS-PHASE5R-RESEARCH"
...
GOVERNANCE_PATHS = {
    ...
    "phase5.pass_publication": "docs/superpowers/governance/2026-08-15-phase-5-pass-publication.json",
    "phase5r.design_specification": (
        "docs/superpowers/specs/2026-08-15-phase-5r-research-model-infrastructure-design.md"
    ),
    "phase5r.governance_approvals": (
        "docs/superpowers/governance/2026-08-15-phase-5r-governance-approvals.json"
    ),
    "phase5r.governance_plan": (
        "docs/superpowers/plans/2026-08-15-phase-5r-research-model-infrastructure.md"
    ),
    "phase5r.implementation_activation": (
        "docs/superpowers/governance/2026-08-15-phase-5r-implementation-activation.json"
    ),
    "phase5r.implementation_authorization": (
        "docs/superpowers/governance/2026-08-15-phase-5r-implementation-authorization.json"
    ),
}
...
def load_review_output(review_dir: Path) -> dict[str, object]:
    return load_json(review_dir / "phase5r.ai_review_output.json")
...
def build_approval_records(*, approved_at: str, candidate_root: str) -> dict[str, object]:
    return {
                    ...
                    canonical_bytes(
                        {
                            ...
                            "approved_logical_id": "phase5r.candidate_evidence_root",
                            "approved_sha256": candidate_root,
                            "approval_scope": "PHASE_5R_CANDIDATE_EVIDENCE_ROOT_ACCEPTANCE",
                            "status": "APPROVED",
                        }
                    )
                ),
        ...
        "logical_id": "phase5r.approval_records",
...
def build_coverage(
    ...
    *,
    ...
    return {
        ...
        "logical_id": "phase5r.ai_review_coverage",
...
def build_ai_review_runs(
    ...
    *,
    ...
    adv_run = load_json(adversarial_dir / "phase5r.ai_review_run.json")
    int_run = load_json(integrity_dir / "phase5r.ai_review_run.json")
    return {
        "candidate_evidence_root": candidate_root,
        "review_runs": sorted(
            [
                {
                    ...
                    "output_repository_relative_path": repo_relative(
                        adversarial_dir / "phase5r.ai_review_output.json"
                    ),
                    ...
                    "run_record_repository_relative_path": repo_relative(
                        adversarial_dir / "phase5r.ai_review_run.json"
                    ),
...
def build_index_members(bundle: Path, postreview: Path) -> list[dict[str, object]]:
    ...
    postreview_map = {
        "phase5r.ai_review_runs": postreview / "phase5r.ai_review_runs.json",
        "phase5r.ai_review_coverage": postreview / "phase5r.ai_review_coverage.json",
        "phase5r.approval_records": postreview / "phase5r.approval_records.json",
    }
    ...
    candidate_ids = [
        row["logical_id"] for row in members if row["logical_id"] != "phase5r.candidate_evidence_root"
    ]
...
def build_acceptance_index(members: list[dict[str, object]], *, candidate_root: str) -> dict[str, object]:
    provisional = {
        ...
        "logical_id": "phase5r.acceptance_index",
...
def build_final_result(
    *,
    ...
    provisional = {
        ...
        "logical_id": "phase5r.final_acceptance_result",
        "outcome": outcome,
...
def build_all(
    *,
    ...
    write_canonical(
        postreview / "phase5r.approval_records.json",
    ...
    write_canonical(postreview / "phase5r.ai_review_coverage.json", coverage_doc)
    write_canonical(
        postreview / "phase5r.ai_review_runs.json",
    ...
    write_canonical(postreview / "phase5r.acceptance_index.json", index_doc)
    ...
    write_canonical(postreview / "phase5r.final_acceptance_result.json", final_doc)
    return {
