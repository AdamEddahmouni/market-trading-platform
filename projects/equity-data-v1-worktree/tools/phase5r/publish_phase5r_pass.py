...
# Filled after first pipeline run.
RUN_ID = "3E468961BF508D7257BE3C2C717F46BEE5A0D583A033A1E8EAB75790F4D802CA"
CANDIDATE_ROOT = "B80779E68C39AFDAF534CD9233D990C0C1DD139ABA305B94B9B36B21162BD93D"
POSTREVIEW_DIR = ROOT / "evidence/phase5r/postreview-pass"
...
def publish(*, publication_path: Path) -> dict[str, object]:
    final_path = POSTREVIEW_DIR / "phase5r.final_acceptance_result.json"
    index_path = POSTREVIEW_DIR / "phase5r.acceptance_index.json"
    ...
    if manifest.get("phase5r_status") == "PASS":
        raise ValueError("Phase 5R PASS already published")

    authority_at_publication = write_canonical_json(
        AUTHORITY_PATH,
        {**manifest, "phase5r_status": "PASS"},
    )
    ...
    publication_doc = {
        "acceptance_index": {
            ...
            "repository_relative_path": "evidence/phase5r/postreview-pass/phase5r.acceptance_index.json",
            "root_hash": index_doc.get("root_hash"),
            "sha256": sha256_bytes(index_path.read_bytes()),
        },
        "artifact_type": "PHASE_5R_PASS_PUBLICATION",
        ...
        "effect": (
            "Publishes Phase 5R PASS for research dataset manifests, walk-forward evaluation, "
            "typed forecast interfaces, and model artifact identity on the admitted equity intraday "
            "source. Phase 6 remains unauthorized."
        ),
        "final_acceptance_result": {
            "final_result_id": final_doc.get("final_result_id"),
            "outcome": final_doc.get("outcome"),
            "repository_relative_path": "evidence/phase5r/postreview-pass/phase5r.final_acceptance_result.json",
            "sha256": sha256_bytes(final_path.read_bytes()),
        },
        "logical_id": "phase5r.pass_publication",
        "principal_id": "PROJECT-PRINCIPAL-001",
        "published_at": "2026-08-15T23:35:00.000000000Z",
    ...
    roadmap = roadmap.replace(
        ...
        "| Phase 5R — research/model infrastructure | Under separate authority, may add research datasets, targets, model interfaces, baselines, manifests, PIT walk-forward evaluation, calibration, serialization, and artifact identity. | `PASS` — research/model infrastructure proven on admitted fixture; Phase 6 remains unauthorized |",
    )
