"""Build Phase 15 public_catalyst whale family acceptance evidence."""
...
REGISTRY_PATH = ROOT / "manifests/phase15/assertion-predicates.json"
FIXTURE_MANIFEST = ROOT / "tests/fixtures/providers/catalyst/admission_manifest.json"
EVALUATED_AT = "2026-08-17T23:30:00.000000000Z"
...
from market_platform_foundation.phase15_assertions import aggregate_status, build_registry, create_run_manifest, evaluate_run
...
def _safe003_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_15_SAFE003_REPORT",
        "logical_id": "phase15.safe003_report",
...
def _fixture_report() -> dict[str, object]:
    ...
    if not isinstance(claims, list):
        ...
        for claim in ("catalyst_gate", "confidence_scoring", "lean_direction", "source_attribution"):
            if claim not in claims:
                failures.append(f"MISSING_{claim}")
    return {
        "artifact_type": "PHASE_15_FIXTURE_REPORT",
        "failures": failures,
        "logical_id": "phase15.fixture_report",
        "status": "PASS" if not failures else "FAIL",
    }


def _catalyst_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_15_CATALYST_REPORT",
        "deterministic_envelope_ids": deterministic,
        "event_count": len(first_ids),
        "logical_id": "phase15.catalyst_report",
...
def _pit_report() -> dict[str, object]:
    ...
    return {
        "artifact_type": "PHASE_15_PIT_REPORT",
        "event_count": len(result.events),
        "logical_id": "phase15.pit_report",
...
def _whale_report() -> dict[str, object]:
    ...
    failures: list[str] = []
    if boxl.get("status") != "available" or boxl.get("reason_code") != WHALE_ENTITLED_CATALYST:
        failures.append("BOXL_UNAVAILABLE")
    ...
    return {
        "artifact_type": "PHASE_15_WHALE_REPORT",
        "failures": failures,
        "logical_id": "phase15.whale_report",
        "status": "PASS" if not failures else "FAIL",
    }
...
def _ui_report() -> dict[str, object]:
    ...
    failures: list[str] = []
    if by_id.get("whale.public_catalyst", {}).get("state") != "AVAILABLE":
    ...
    return {
        "artifact_type": "PHASE_15_UI_REPORT",
        "failures": failures,
        "logical_id": "phase15.ui_report",
        "status": "PASS" if not failures else "FAIL",
    }
...
def build_evidence(output_dir: Path) -> dict[str, object]:
    ...
    write_canonical_json(output_dir / "catalyst-report.json", catalyst)
    ...
    observations = {
        "P15-FIX-001": {"failures": fixture.get("failures"), "reason_codes": fixture.get("failures", []), "status": fixture.get("status")},
        "P15-CAT-001": {"reason_codes": [] if catalyst.get("status") == "PASS" else ["P15-CAT-001-FAIL"], "status": catalyst.get("status")},
        "P15-PIT-001": {"reason_codes": [] if pit.get("status") == "PASS" else ["P15-PIT-001-FAIL"], "status": pit.get("status")},
        "P15-WHALE-001": {"failures": whale.get("failures"), "reason_codes": whale.get("failures", []), "status": whale.get("status")},
        "P15-UI-001": {"failures": ui.get("failures"), "reason_codes": ui.get("failures", []), "status": ui.get("status")},
        "SAFE-003": {"reason_codes": safe003.get("reason_codes"), "status": safe003.get("status")},
    }

    members = {
        "phase15.catalyst_report": "catalyst-report.json",
        "phase15.fixture_report": "fixture-report.json",
        "phase15.pit_report": "pit-report.json",
        "phase15.safe003_report": "safe003-report.json",
        "phase15.ui_report": "ui-report.json",
        "phase15.whale_report": "whale-report.json",
    }
    selected_evidence = []
    ...
    manifest_inputs = {
        ...
        "subject_manifest_hash": sha256_bytes(canonical_bytes({"track": "phase15", "root_id": "ROOT-CATALYST-LANE"})),
        "tool_versions": ["phase15.run_phase15_pipeline/1.0.0"],
    }
    ...
    bundle_members = {
        **members,
        "phase15.assertion_aggregate": "assertion-aggregate.json",
        "phase15.assertion_registry": "assertion_registry.json",
        "phase15.assertion_results": "assertion-results.json",
        "phase15.assertion_run_manifest": "assertion-run-manifest.json",
    }
    candidate_members = []
    ...
    candidate_body = {
        ...
        "logical_id": "phase15.candidate_evidence_root",
        "run_id": run_id,
    }
