...
RUN_ID = "B0424C196D7BA6A3398DFDC56C58E9844153B8DA72D8DA4FE79A15AAB7344009"
CANDIDATE_ROOT = "CA0BDAC9DDE7896C5D443BCA94FBDB565DEA79C04CFAA833270EA7B87AC90F78"
POSTREVIEW_DIR = ROOT / "evidence/phase6/postreview-pass"
PUBLICATION_PATH = ROOT / "docs/superpowers/governance/2026-08-15-phase-6-pass-publication.json"
...
def publish(*, publication_path: Path) -> dict[str, object]:
    final_path = POSTREVIEW_DIR / "phase6.final_acceptance_result.json"
    index_path = POSTREVIEW_DIR / "phase6.acceptance_index.json"
    ...
    if manifest.get("phase6_status") == "PASS":
        raise ValueError("Phase 6 PASS already published")

    authority_at_publication = write_canonical_json(
        AUTHORITY_PATH,
        {**manifest, "phase6_status": "PASS"},
    )
    ...
    publication_doc = {
        "acceptance_index": {
            ...
            "repository_relative_path": "evidence/phase6/postreview-pass/phase6.acceptance_index.json",
            "root_hash": index_doc.get("root_hash"),
            "sha256": sha256_bytes(index_path.read_bytes()),
        },
        "artifact_type": "PHASE_6_PASS_PUBLICATION",
        ...
        "effect": (
            "Publishes Phase 6 PASS for preregistered strategy specification, interpretation, "
            "and abstention on the admitted equity intraday source. Phase 7 remains unauthorized."
        ),
        "final_acceptance_result": {
            "final_result_id": final_doc.get("final_result_id"),
            "outcome": final_doc.get("outcome"),
            "repository_relative_path": "evidence/phase6/postreview-pass/phase6.final_acceptance_result.json",
            "sha256": sha256_bytes(final_path.read_bytes()),
        },
        "logical_id": "phase6.pass_publication",
        "principal_id": "PROJECT-PRINCIPAL-001",
        "published_at": "2026-08-16T00:30:00.000000000Z",
