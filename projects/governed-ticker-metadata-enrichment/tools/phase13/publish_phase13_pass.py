...
POSTREVIEW_DIR = ROOT / "evidence/phase13/postreview-pass"
PUBLICATION_PATH = ROOT / "docs/superpowers/governance/2026-08-19-phase-13-pass-publication.json"
...
def publish(*, publication_path: Path, run_id: str, candidate_root: str) -> dict[str, object]:
    final_path = POSTREVIEW_DIR / "phase13.final_acceptance_result.json"
    index_path = POSTREVIEW_DIR / "phase13.acceptance_index.json"
    ...
    if manifest.get("phase13_status") == "PASS":
        raise ValueError("Phase 13 PASS already published")

    authority_at_publication = write_canonical_json(
        AUTHORITY_PATH,
        {**manifest, "phase13_status": "PASS"},
    )
    ...
    publication_doc = {
        "acceptance_index": {
            ...
            "repository_relative_path": "evidence/phase13/postreview-pass/phase13.acceptance_index.json",
            "root_hash": index_doc.get("root_hash"),
            "sha256": sha256_bytes(index_path.read_bytes()),
        },
        "artifact_type": "PHASE_13_PASS_PUBLICATION",
        ...
        "effect": (
            "Publishes Phase 13 PASS for fixture-first order_book whale family on "
            "ADMITTED-L2-NVDA-001."
        ),
        "final_acceptance_result": {
            "final_result_id": final_doc.get("final_result_id"),
            "outcome": final_doc.get("outcome"),
            "repository_relative_path": "evidence/phase13/postreview-pass/phase13.final_acceptance_result.json",
            "sha256": sha256_bytes(final_path.read_bytes()),
        },
        "logical_id": "phase13.pass_publication",
        "principal_id": "PROJECT-PRINCIPAL-001",
        "published_at": "2026-08-19T12:35:00.000000000Z",
