"""Build Phase 2 postreview gate artifacts."""
...
from tools.phase2.acceptance_algorithms import (
...
DEFAULT_POSTREVIEW = ROOT / "evidence/phase2/postreview-pass"
DEFAULT_ADVERSARIAL = ROOT / "evidence/phase2/review-runs/ADVERSARIAL-PASS-PHASE2-CONTRACTS"
DEFAULT_INTEGRITY = ROOT / "evidence/phase2/review-runs/INTEGRITY-PASS-PHASE2-CONTRACTS"
...
GOVERNANCE_PATHS = {
    ...
    "phase2.design_specification": (
        "docs/superpowers/specs/2026-08-15-phase-2-canonical-contracts-and-replay-design.md"
    ),
    "phase2.governance_approvals": (
        "docs/superpowers/governance/2026-08-15-phase-2-governance-approvals.json"
    ),
    "phase2.governance_plan": (
        "docs/superpowers/plans/2026-08-15-phase-2-canonical-contracts-and-replay.md"
    ),
    "phase2.implementation_activation": (
        "docs/superpowers/governance/2026-08-15-phase-2-implementation-activation.json"
    ),
    "phase2.implementation_authorization": (
        "docs/superpowers/governance/2026-08-15-phase-2-implementation-authorization.json"
    ),
}
...
def load_review_output(review_dir: Path) -> dict[str, object]:
    return load_json(review_dir / "phase2.ai_review_output.json")
...
def build_approval_records(*, approved_at: str) -> dict[str, object]:
    return {
                    ...
                    canonical_bytes(
                        {
                            ...
                            "approved_logical_id": "phase2.candidate_evidence_root",
                            "approved_sha256": CANDIDATE_ROOT,
                            "approval_scope": "PHASE_2_CANDIDATE_EVIDENCE_ROOT_ACCEPTANCE",
                            "status": "APPROVED",
                        }
                    )
                ),
        ...
        "logical_id": "phase2.approval_records",
...
def build_coverage(adversarial: dict[str, object], integrity: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "coverage_logical_ids_union": sorted(
            ...
            | set(BUNDLE_FILES)
            | {"phase2.candidate_evidence_root"}
        ),
        "disqualification_reason_codes": [],
        "logical_id": "phase2.ai_review_coverage",
...
def build_ai_review_runs(
    ...
    adv_run = load_json(adversarial_dir / "phase2.ai_review_run.json")
    int_run = load_json(integrity_dir / "phase2.ai_review_run.json")
    return {
        "candidate_evidence_root": CANDIDATE_ROOT,
        "review_runs": sorted(
            [
                {
                    "completed_at": adv_run.get("completed_at", "2026-08-15T16:30:00.000000000Z"),
                    "output_repository_relative_path": repo_relative(
                        adversarial_dir / "phase2.ai_review_output.json"
                    ),
                    ...
                    "run_record_repository_relative_path": repo_relative(
                        adversarial_dir / "phase2.ai_review_run.json"
                    ),
                    "terminal_state": adv_run["terminal_state"],
                },
                {
                    "completed_at": int_run.get("completed_at", "2026-08-15T16:30:00.000000000Z"),
                    "output_repository_relative_path": repo_relative(
                        integrity_dir / "phase2.ai_review_output.json"
                    ),
                    "review_class": integrity["review_class"],
                    "review_output_hash": int_run["review_output_hash"],
                    "review_run_id": int_run["review_run_id"],
                    "run_record_repository_relative_path": repo_relative(
                        integrity_dir / "phase2.ai_review_run.json"
                    ),
...
def build_index_members(bundle: Path, postreview: Path) -> list[dict[str, object]]:
    ...
    postreview_map = {
        "phase2.ai_review_runs": postreview / "phase2.ai_review_runs.json",
        "phase2.ai_review_coverage": postreview / "phase2.ai_review_coverage.json",
        "phase2.approval_records": postreview / "phase2.approval_records.json",
    }
    ...
    candidate_ids = [row["logical_id"] for row in members if row["logical_id"] != "phase2.candidate_evidence_root"]
...
def build_acceptance_index(members: list[dict[str, object]]) -> dict[str, object]:
    provisional = {
        ...
        "logical_id": "phase2.acceptance_index",
...
def build_final_result(
    *,
    ...
    provisional = {
        ...
        "logical_id": "phase2.final_acceptance_result",
        "outcome": outcome,
...
def build_all(
    *,
    ...
    write_canonical(postreview / "phase2.approval_records.json", build_approval_records(approved_at=approved_at))
    coverage_doc = build_coverage(adversarial, integrity)
    write_canonical(postreview / "phase2.ai_review_coverage.json", coverage_doc)
    write_canonical(
        postreview / "phase2.ai_review_runs.json",
    ...
    write_canonical(postreview / "phase2.acceptance_index.json", index_doc)
    ...
    write_canonical(postreview / "phase2.final_acceptance_result.json", final_doc)
    return {
...
def main() -> int:
    ...
    parser.add_argument("--approved-at", default="2026-08-15T16:30:00.000000000Z")
    parser.add_argument("--completed-at", default="2026-08-15T16:32:00.000000000Z")
