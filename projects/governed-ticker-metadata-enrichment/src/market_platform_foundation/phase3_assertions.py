"""Phase 3 adapter integrity assertion registry and evaluator."""
...
MANDATORY_IDS = ("ADP-001", "ADP-002", "SAFE-001", "SAFE-002")

_ROLES = {
    "ADP-001": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
    "ADP-002": (["ARCHITECTURE_LEAD"], ["PROJECT_OWNER"], ["INDEPENDENT_REVIEWER"]),
...
def build_registry(path: Path) -> dict[str, object]:
    ...
    if not isinstance(raw, dict) or raw.get("registry_version") != "1.0.0":
        raise ValueError("unsupported phase3 assertion registry")
