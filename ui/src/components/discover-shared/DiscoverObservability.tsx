...
export type DiscoverObservabilityProps = {
...
export function DiscoverObservability({
  allowMutations = false,
  autoRefreshOnMount = false,
}: DiscoverObservabilityProps) {
  ...
  const openWorkspace = async (instrumentId: string) => {
    if (allowMutations) {
      try {
  ...
  useEffect(() => {
    ...
    if (!document.hidden) {
      void readMixed().then(() => {
        if (autoRefreshOnMount) void refreshMixed();
      });
    }
  ...
  useEffect(() => {
    ...
    allowMutations,
    mode,
    mixed?.poll_interval_seconds,
    mixed?.refresh_interval_seconds,
    readMixed,
    refreshMixed,
  ]);
    ...
    <div className="discover-observability">
      ...
      {mode === "MIXED" ? (
            ...
            <div className="discover-provider-health" aria-label="Provider health">
              {(mixed?.provider_health ?? []).map((provider) => (
                <span
                  ...
                  className={`provider-state provider-${provider.connection.toLowerCase()}`}
                >
                  <i aria-hidden="true" />
                  {displayProvider(provider.provider)} <strong>{provider.connection}</strong>
                  {provider.subscribed_candidates != null
                    ? ` · ${provider.subscribed_candidates} quotes`
                    : ""}
                </span>
              ))}
              ...
              >
                ...
                <div className="discover-data-state">
                  ...
                  {candidate.market.bid_price != null && candidate.market.ask_price != null ? (
                    <span>
                      {formatNumber(candidate.market.bid_price)} × {formatNumber(candidate.market.ask_price)}
                    </span>
                  ) : null}
                </div>
                <div className="discover-row-actions">
                  ...
                  <details>
                    ...
                    <div className="discover-evidence">
                      <p>
                        <strong>Why:</strong>{" "}
                        {(candidate.supporting_evidence ?? candidate.matched_reasons).join(" · ")}
                      </p>
                      <p>
                        <strong>Ranking:</strong> {candidate.ranking_reasons.join(" · ")}
                      </p>
                      <p>
                        <strong>Screens:</strong> {candidate.screen_matches.join(" · ")}
                      </p>
                      {candidate.caveats && candidate.caveats.length > 0 ? (
                        <p>
                          <strong>Caveats:</strong> {candidate.caveats.join(" · ")}
                        </p>
                      ) : null}
                      <JsonDetailPanel title="Attention components" value={candidate.attention_components} />
                    </div>
                  </details>
                </div>
              </article>
        ...
        <section aria-label="Single screen diagnostics">
            ...
            {singleCandidates.map((candidate) => (
              <article key={candidate.instrument_id} className="discover-card">
                ...
                <div className="discover-why">
                  <strong>Why:</strong> {candidate.matched_reasons.join(" · ")}
                </div>
                <div className="discover-metrics">
                  {candidate.metrics.rel_volume != null ? (
                    <span>RVOL {String(candidate.metrics.rel_volume)}</span>
                  ) : null}
                  ...
                  {candidate.metrics.short_float_pct != null ? (
                    <span>Short {formatPercent(candidate.metrics.short_float_pct)}</span>
                  ) : null}
                </div>
          ...
          {run?.screen ? (
            <JsonDetailPanel title="Screen inspector" value={run.screen} />
          ) : null}
        </section>
      )}
    </div>
