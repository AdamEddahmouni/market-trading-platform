...
# Add expected contract column
df['expected_symbol'] = df['timestamp'].apply(get_expected_contract)

# KEEP only rows where the symbol matches the expected contract
df = df[df['symbol'] == df['expected_symbol']]

# Drop the helper column
df = df.drop(columns=["expected_symbol"])
